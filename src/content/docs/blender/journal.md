...
|---|---|---|---|---|
| Does the same `bpy` script give the same report on Windows, Linux and macOS? | Not written in advance: `check.sh` printed the render hash without comparing it, precisely because nobody knew whether it would match. | The reports are identical on the three runners, the voxel remesh included, and the hash of the 16-sample render matches on x86-64 and on Apple Silicon. | Confirmed | [2026-09-16](#2026-09-16--scripts-ci-and-what-they-showed) · [`check.sh`](https://github.com/spareilleux/learn/blob/0c94215/code/blender/check.sh) |
| Does a model generated from an image carry its lighting painted into the texture? | Written in advance: yes, and it was the failure the pipeline was built to show. | This workflow outputs shape only: no UV map, no texture, one empty material. The expected failure could not happen. | Refuted | [2026-09-17](#2026-09-17--models-generated-in-comfyui-cleaned-in-blender) · [`glb_pipeline.py`](https://github.com/spareilleux/learn/blob/0c94215/code/blender/scripts/glb_pipeline.py) |
| Does Decimate in Collapse mode reach a 20,000-triangle target on a surface-net mesh? | Not written in advance: the pipeline asked for a ratio and printed only the ratio it had asked for, which is that assumption made visible. | 43,302 triangles on the metronome and 317,253 on the gramophone: missed on both, because of 19,084 and 189,748 non-manifold edges. | Refuted | [2026-09-17](#2026-09-17--models-generated-in-comfyui-cleaned-in-blender) · [`glb_pipeline.py`](https://github.com/spareilleux/learn/blob/0c94215/code/blender/scripts/glb_pipeline.py) |
| Does a voxel remesh first give a mesh Decimate can bring to target? | Not written in advance: the remesh was added in answer to the row above, with the loss of detail below one voxel expected. | 20,000 triangles exactly on both, 0 non-manifold edges, no boundary. Thin walls break into crumbs, which a second floater pass removes. | Confirmed | [2026-09-17](#2026-09-17--models-generated-in-comfyui-cleaned-in-blender) · [`glb_pipeline.py`](https://github.com/spareilleux/learn/blob/0c94215/code/blender/scripts/glb_pipeline.py) |
