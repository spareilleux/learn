...
- **A tensor used twice** receives both contributions: the product of `h` with itself gives `h` a gradient of `h` from each side, so the gradient is **twice** `h`, `[4, -2, 8]`, for `w = [1, -2, 3]`.
