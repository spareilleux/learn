...
rustc, cargo, Tauri and the crates around them. One row is the only finding in this repository that was already open upstream when the course hit it, and the Status column names the issue rather than implying the course filed it. Nothing else here has been reported.

| Expected | What happens | Where | Measure | Status |
|---|---|---|---|---|
| `cargo test` runs the tests of a Tauri application | Every test executable died before running a test: `tauri-build` embeds the Common Controls v6 manifest in the binaries only | `tauri-build`, Tauri 2.11.5, Windows | `STATUS_ENTRYPOINT_NOT_FOUND` | Open upstream since May 2025 as [tauri#13419](https://github.com/tauri-apps/tauri/issues/13419), not opened by this course. Worked around in `build.rs` with `cargo:rustc-link-arg-tests` [2026-09-15](#2026-09-15--lessons-16-to-18-desktop-applications-with-tauri) |
| `compile_fail,E0382` checks that the error code is the one named | On stable it only checks that compilation fails | `cargo test --doc`, rustc 1.94.0 | A doctest marked `compile_fail,E0999` around a use-after-move still passed. `cargo +nightly test --doc` does compare the codes | Reproduced; documented limitation. CI now also runs nightly [2026-09-13](#2026-09-13--lessons-1-to-4) |
| Tauri's documentation compiles and targets Tauri 2 | The *Calling Rust* page's error example does not compile, its `AppHandle` example uses Tauri 1 APIs, and the *State management* page describes a panic that 2.11.5 does not do | tauri-docs at `a6b59b7` | `Ok(format!(value))`; `GlobalShortcutManager` and `app_dir`; forgetting `.manage()` compiles and the call is rejected at run time with `state not managed for field …`, only `Manager::state` panics | Reproduced against 2.11.5, not reported [2026-09-15](#2026-09-15--lessons-16-to-18-desktop-applications-with-tauri) |
| `create-tauri-app` scaffolds a current desktop project | It prints a mobile command as the first step of a desktop project, and its template is out of date | `create-tauri-app` 4.7.4 | `npm run tauri android init`; the template uses edition 2021 and `"csp": null` | Reproduced, not reported [2026-09-15](#2026-09-15--lessons-16-to-18-desktop-applications-with-tauri) |
| The route Tauri's WebDriver page recommends is the fast one | It is about ninety times slower | `@wdio/tauri-service` | Two tests took 1 min 30 s, because it waits 5 s before each command for a second plugin the course does not use. Plain WebdriverIO against the embedded server runs them in about a second | Reproduced, not reported [2026-09-16](#2026-09-16--lessons-19-to-21-frontend-security-tests-and-packaging) |
| `tauri dev` enforces the CSP | It does not | Tauri 2.11.5 with Vite | A `fetch` to a remote API returned `200` in dev and was blocked by a `connect-src` violation in the release build | Reproduced; a development-only false sense of safety [2026-09-16](#2026-09-16--lessons-19-to-21-frontend-security-tests-and-packaging) |
| The mock runtime's page origin is the same everywhere | `http://tauri.localhost` is the origin on Windows and Android only; macOS and Linux use `tauri://localhost` | Tauri 2.11.5 mock runtime | On the first CI run the capabilities rejected every call with `not allowed. Plugin not found` | Reproduced on CI, fixed in the course's code [2026-09-15](#2026-09-15--lessons-16-to-18-desktop-applications-with-tauri) |
| A refused call refuses with the command's error type | Tauri's own refusals reject with a string, and the two builds word them differently | Tauri 2.11.5 ACL | The page showed `undefined: undefined`. Dev: `dialog.save not allowed. Permissions associated with this command: …`; release: `Command plugin:store|load not allowed by ACL` | Reproduced [2026-09-15](#2026-09-15--lessons-16-to-18-desktop-applications-with-tauri) · [2026-09-16](#2026-09-16--lessons-19-to-21-frontend-security-tests-and-packaging) |
| `cargo fmt --check --manifest-path …` behaves the same on any recent cargo | It works locally and fails on the runners | Cargo 1.94.0 locally, 1.98.1 on the runners | `Failed to find targets`. CI now runs `cargo fmt --check` from each crate's folder | Reproduced, a cross-version change, not reported [2026-09-13](#2026-09-13--lessons-13-to-15-and-three-operating-systems) |
| ts-rs and serde_json agree on a `u128` | ts-rs types it as `bigint` while serde_json sends a plain number | ts-rs with serde_json | `tsc` rejected `elapsedMs / 1000` for lesson 18's `elapsed_ms: u128`. Worked around with `#[ts(type = "number")]` | Reproduced [2026-09-16](#2026-09-16--lessons-19-to-21-frontend-security-tests-and-packaging) |
| Vitest type-checks the tests it runs | It does not | Vitest | A test built with `elapsedMs: 3` passed while `tsc` rejected the same file | By design, documented [2026-09-16](#2026-09-16--lessons-19-to-21-frontend-security-tests-and-packaging) |
| A synchronous command does not freeze the application | It freezes the native window, though the webview keeps running | Tauri 2.11.5, Windows | During a one-second search the page's timers kept ticking, but the native window did not answer `WM_NULL` for 945 ms and every other command waited | Reproduced; by design [2026-09-15](#2026-09-15--lessons-16-to-18-desktop-applications-with-tauri) |
| The `RefCell` panic message matches published material | It changed | Rust 1.94 standard library | 1.94 says `RefCell already borrowed`; older material quotes `already borrowed: BorrowMutError` | Reproduced; doc rot across the ecosystem [2026-09-13](#2026-09-13--lessons-5-to-8) |
| A recursive enum gives one clear error | It gives two, one of them about a cycle in the compiler's own query | rustc 1.94.0 | `E0391` in addition to `E0072` | Reproduced; diagnostics noise [2026-09-13](#2026-09-13--lessons-5-to-8) |
| `[lints] missing_docs = "warn"` covers the library | It also covers the integration tests, because each file in `tests/` is its own crate | Cargo lints | Each needs its own `//!` line | By design, surprising [2026-09-13](#2026-09-13--lessons-9-to-12) |
| A forgotten `.await` still runs the work | The code never runs at all, the opposite of C#, where the task starts anyway | async Rust | No output, no error | By design; the single biggest trap for a C# reader [2026-09-13](#2026-09-13--lessons-9-to-12) |
| `tauri build` uses the packaging tools already installed | It downloads them itself on first use, with hash checks | Tauri CLI 2.11.4 | WiX 3.14 and NSIS 3.11 | Reproduced; a supply-chain fact worth knowing [2026-09-16](#2026-09-16--lessons-19-to-21-frontend-security-tests-and-packaging) |
| Twenty seconds is enough for the embedded WebDriver server on CI | Not on Linux, and far too generous on Windows | CI runners | `ubuntu-latest` gave no answer within 20 s; at 60 s, WebdriverIO's own CI default, it answered after about 25 s. `windows-latest` answered in 394 ms, before the page had attached its handlers, so the first test clicked a plain HTML form | Reproduced on CI, both ends fixed [2026-09-16](#2026-09-16--lessons-19-to-21-frontend-security-tests-and-packaging) |

## Experiments

Five of these seven were wrong, and that is the point: in this course a prediction is usually written into a doctest or a lesson claim, so the build fails when it is wrong instead of quietly being rewritten. The one hypothesis that was only recalled afterwards says so.

| Question | Hypothesis | Result | Verdict | Where |
|---|---|---|---|---|
| Does `compile_fail,E0382` check the error code? | Written as the journal's own question, before the answer | A doctest marked `compile_fail,E0999` around a use-after-move still passed on stable 1.94.0 | Refuted; CI now also runs nightly, which does compare the codes | [2026-09-13](#2026-09-13--lessons-1-to-4) |
| Does `pricing_sum([19.99, 5.0, 12.5])` equal 37.49? | Written into the lesson 15 FFI test as an assertion, before it ran | The real `f64` sum is 37.489999999999995. C# had printed 37.49 only because of its `F2` format | Refuted, by a test that failed | [2026-09-13](#2026-09-13--lessons-13-to-15-and-three-operating-systems) |
| Can `cheapest` take a `Vec<Box<dyn Priced>>` with a `T: Priced + ?Sized` bound? | Claimed in lesson 7 before it was compiled | Wrong: `&[T]` requires `T: Sized`. The working version implements `Priced` for `Box<dyn Priced>` | Refuted | [2026-09-13](#2026-09-13--lessons-5-to-8) |
| Does `[LibraryImport]` marshal a `bool` as a four-byte `BOOL` by default? | Written into the material before the build | It has no default at all: the build fails with `SYSLIB1051` until a `[MarshalAs]` is added. That was `[DllImport]`'s behaviour | Refuted | [2026-09-13](#2026-09-13--lessons-13-to-15-and-three-operating-systems) |
| How does a rayon `map` that mutates a captured counter fail? | Assumed to fail on `Send`/`Sync`; the assumption is recorded after the fact | It fails earlier: rayon's closures are `Fn`, so it is `E0594: cannot assign to a captured variable in a Fn closure` | Refuted | [2026-09-13](#2026-09-13--lessons-5-to-8) |
| Do three concurrent tokio sleeps finish within 290 ms? | Asserted in a lesson 13 doctest before CI ran it | True on the author's machine, but a timing assertion is a flaky test on shared runners. The test now checks only the results | Confirmed locally, withdrawn as an assertion | [2026-09-13](#2026-09-13--lessons-9-to-12) |
| Does integer overflow behave differently in the two profiles? | A documented claim, deliberately checked rather than repeated | `255u8 + 1` panics in debug and wraps in release. C# wraps in both unless `checked` | Confirmed | [2026-09-13](#2026-09-13--lessons-1-to-4) |
