...
| The OPTIC-K index export is reproducible from one commit | The first export of the evening differs from the later ones in 266 of 313,047 entries, from the same GA commit | OPTIC-K export | 266 entries of 313,047, keyed by instrument and diagram | Fixed upstream by [#730](https://github.com/GuitarAlchemist/ga/pull/730), merged 2026-09-24. The cause was the parallel voicing generator, whose output order followed thread scheduling; two guitar exports of GA's main hashed differently before, identically after [2026-09-17](#2026-09-17--appendix-2-profiling-ga-with-pull-requests), [2026-09-24](#2026-09-24--upstream-fixes-checked-again) |
| `VoicingGenerator`'s parallel path is faster than its sequential one | It was twice as slow: a single consumer built every diagram and dropped the duplicates | [`VoicingGenerator.cs`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Domain.Services/Fretboard/Voicings/Generation/VoicingGenerator.cs#L180-L249) | 1,419 ms against 711 ms for 667,125 voicings | Fixed upstream by [#730](https://github.com/GuitarAlchemist/ga/pull/730): the producers do that work, 649-679 ms against 786-861 ms sequential, measured for the PR; not re-measured in this course [2026-09-17](#2026-09-17--appendix-2-profiling-ga-with-pull-requests), [2026-09-24](#2026-09-24--upstream-fixes-checked-again) |
| `PitchClassSet.GetCompatibleKeys` is cheap enough to call per voicing | It rebuilt the 30 keys and their pitch-class sets on every call | [`PitchClassSet.cs`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Domain.Core/Theory/Atonal/PitchClassSet.cs#L592-L595) | 13.5% of the analysis profile; 180.5 ms and 110,116 bytes per call over 4,096 calls | Fixed upstream by [#730](https://github.com/GuitarAlchemist/ga/pull/730): a subset test on 12-bit masks, computed once per set, 0.1 ms and 0 bytes; not re-measured in this course [2026-09-17](#2026-09-17--appendix-2-profiling-ga-with-pull-requests), [2026-09-24](#2026-09-24--upstream-fixes-checked-again) |
| `ValueObjectUtils<TSelf>.Items` returns the collection it holds | It creates a new 32-byte `ValueObjectCollection<TSelf>` on each call | [`ValueObjectUtils.cs`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Core/ValueObjects/ValueObjectUtils.cs#L10) | 32 bytes per call | Fixed upstream by [#730](https://github.com/GuitarAlchemist/ga/pull/730): one immutable collection, 0 bytes per read [2026-09-16](#2026-09-16--dogfooding-gas-value-object-interfaces), [2026-09-24](#2026-09-24--upstream-fixes-checked-again) |
| `IRangeValueObject<TSelf>.EnsureValueInRange` normalizes over the real range | It normalizes with a range size computed off by one | [`IRangeValueObject.cs`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Core/Abstractions/IRangeValueObject.cs#L55-L63) | Wrong value at the boundary | Fixed upstream in [#598](https://github.com/GuitarAlchemist/ga/pull/598), merged 2026-09-24, without a test; [#731](https://github.com/GuitarAlchemist/ga/pull/731) adds one, which fails on the old code [2026-09-16](#2026-09-16--dogfooding-gas-value-object-interfaces), [2026-09-24](#2026-09-24--upstream-fixes-checked-again) |
...
| `ValueObjectCache<T>` builds what its callers use | It builds two `FrozenSet`s at first use that nothing reads | [`ValueObjectCache.cs`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Core/ValueObjects/ValueObjectCache.cs#L38-L52) | Two sets built, zero readers | Fixed upstream by [#730](https://github.com/GuitarAlchemist/ga/pull/730): the two sets are built on first use [2026-09-16](#2026-09-16--dogfooding-gas-value-object-interfaces), [2026-09-24](#2026-09-24--upstream-fixes-checked-again) |
...
| `PitchClassSetId.ItemsSpan` returns a view of the ids | It copied the 4,096 ids into a new array on every call: `Items` is a compiler-generated read-only list, so the `is PitchClassSetId[]` test was always false | [`PitchClassSetId.cs#L59-L71`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Domain.Core/Theory/Atonal/PitchClassSetId.cs#L59-L71) | 16,408 bytes per call | Fixed upstream by [#687](https://github.com/GuitarAlchemist/ga/pull/687), merged 2026-09-20; not re-measured [2026-09-14](#2026-09-14--dogfooding-what-the-lessons-found-in-ga), [2026-09-24](#2026-09-24--upstream-fixes-checked-again) |
| `PitchClass`'s `-` operator costs about what `(a - b + 12) % 12` costs | It looked the result up in a `FrozenDictionary` of 144 tuples | [`PitchClass.cs#L110-L137`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Domain.Core/Theory/Atonal/PitchClass.cs#L110-L137) | 583 ns for the 144 pairs, against 134 ns for the arithmetic and 41.5 ns for a flat array | Fixed upstream by [#687](https://github.com/GuitarAlchemist/ga/pull/687) with a flat 144-entry table; not re-measured [2026-09-14](#2026-09-14--dogfooding-what-the-lessons-found-in-ga), [2026-09-24](#2026-09-24--upstream-fixes-checked-again) |
| `Try.OfAsync` can be blocked on, and cancelled | It awaited without `ConfigureAwait(false)`, so blocking on it under a single-threaded synchronization context deadlocked, and it turned `OperationCanceledException` into an ordinary failure | [`Try.cs#L62-L73`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Core/Functional/Try.cs#L62-L73) | Shown in lesson 3 | Fixed upstream by [#687](https://github.com/GuitarAlchemist/ga/pull/687) [2026-09-14](#2026-09-14--dogfooding-what-the-lessons-found-in-ga), [2026-09-24](#2026-09-24--upstream-fixes-checked-again) |
| `LazyWithExpiration<T>` costs nothing while it waits | It parked one thread-pool thread in `Thread.Sleep` per value | [`LazyWithExpiration.cs#L20-L40`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Core/Utilities/LazyWithExpiration.cs#L20-L40) | 64 values expiring after one second delayed an unrelated `Task.Run` by 2 s on my machine and 10 to 12 s on the 3- and 4-core runners | Fixed upstream by [#687](https://github.com/GuitarAlchemist/ga/pull/687): the expiration is a timestamp checked on access [2026-09-14](#2026-09-14--dogfooding-what-the-lessons-found-in-ga), [2026-09-24](#2026-09-24--upstream-fixes-checked-again) |
...
- The four findings of lessons 1-4 were fixed upstream by [#687](https://github.com/GuitarAlchemist/ga/pull/687), merged 2026-09-20, each with a test that fails on the old code: `ItemsSpan` returns its backing array, `PitchClass`'s `-` reads a flat 144-entry table (about 42 ns in this course's benchmark, according to the PR), `Try.OfAsync` uses `ConfigureAwait(false)` and lets cancellation propagate, and `LazyWithExpiration<T>` checks a timestamp on access. Checked in the source at GA [`27a1257`](https://github.com/GuitarAlchemist/ga/commit/27a1257f7fe5478c4b1bcb6b86ebe101707ac935). The course stays pinned to `a826864`, so its benchmarks still measure the old code; I haven't run them against the fix (*to verify*).
- The PR notes one change of behaviour: `MonadicServiceBase`, in GaApi and GA.MusicTheory.Service, now lets a request's cancellation propagate instead of returning a failed `Try`.
- These four were only listed in the 2026-09-14 entry; they now have rows in the QA table.
- [#730](https://github.com/GuitarAlchemist/ga/pull/730), merged 2026-09-24, fixes the performance findings still open, and the export that was not reproducible. Measured for the PR on one machine, Release, 4,096 calls per member after a warm-up; I haven't re-run this course's benchmarks against it (*to verify*):
  - `PitchClassSet.ClosestDiatonicKey`: 308.7 ms and 175,699 bytes per call before, 1.0 ms and 0 bytes after; `GetCompatibleKeys`: 180.5 ms and 110,116 bytes, then 0.1 ms and 0 bytes; `ToNormalForm` and `IsNormalForm` read a 4,096-entry table. A test compares each with the old implementation, kept as an oracle, on all 4,096 sets, tie-breaks included.
  - `ValueObjectUtils<TSelf>.Items` returns one collection, and `Values`, read through `IReadOnlyList<int>`, no longer boxes 24 bytes per read on nine types.
  - `VoicingGenerator`'s parallel path now emits its windows in fret order, so its stream is the sequential one. That order was the cause of the export difference: the export keeps the first of equally cheap voicings. Two guitar exports of GA's main hashed `776c849e…` and `d0a7d1d9…`; with the fix, `138603e8…` both times.
- `EnsureValueInRange` was fixed inside [#598](https://github.com/GuitarAlchemist/ga/pull/598), a feature PR, with no test. [#731](https://github.com/GuitarAlchemist/ga/pull/731) adds one: 9 tests pass on main, and 5 of them fail with the file from `a826864`.
