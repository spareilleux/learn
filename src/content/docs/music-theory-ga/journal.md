...
The forty divergences between this course's arithmetic and Guitar Alchemist's, the nineteen defects they collapse into and the verdict on each are in [Appendix C](/learn/music-theory-ga/appendix-ga-findings/); sixteen of them were merged upstream in [GA #711](https://github.com/GuitarAlchemist/ga/pull/711), and three stay as documented design limits. This table is what the appendix does not cover: findings outside the executable comparison. The last row was read rather than run, and says so.

| Expected | What happens | Where | Measure | Status |
|---|---|---|---|---|
| `getAllInstruments()` returns the instruments the YAML holds | It returns one, and two tunings, falling back to hard-coded guitar values without an error or a log line | `InstrumentsYaml`/`InstrumentsConfig`, `GA.Business.Config` at `a826864` | The file holds 122 instruments and 280 tunings; the loader returned 1 and 2. Seven of the 280 are not pitches at all: two pedal steel entries begin with their tuning's name, one with the word `Tuning`, one with the instrument's name, one keeps part of its name | Fixed upstream by [#734](https://github.com/GuitarAlchemist/ga/pull/734), merged 2026-09-24: the file is read as a mapping, 122 instruments and 280 tunings, and a failure now says so on stderr instead of silently answering with the built-in guitar. Nine tunings were restored from `Tunings.txt`; the two harp guitar entries keep their `|` [2026-09-15](#2026-09-15--a-verdict-on-every-divergence-the-ukulele-and-the-bass-three-appendices), [2026-09-24](#2026-09-24--upstream-fixes-checked-again) |
| `Tuning.BuildPitchArray` numbers the strings of any tuning correctly | It decides which end is string 1 by comparing the first pitch with the last one only, so a re-entrant tuning comes out backwards | `Tuning.BuildPitchArray` at `a826864` | GA's own two 5-string banjo tunings, whose drone string is written first and sounds higher than the last, are numbered the wrong way round — contradicting `Str`'s own comment that string 1 is the highest | Fixed upstream by [#734](https://github.com/GuitarAlchemist/ga/pull/734): the direction is decided by counting rising against falling steps between adjacent strings, so both banjo tunings number string 1 as the highest [2026-09-15](#2026-09-15--a-verdict-on-every-divergence-the-ukulele-and-the-bass-three-appendices), [2026-09-24](#2026-09-24--upstream-fixes-checked-again) |
| `Fretboard.GetNote`'s documented parameter matches its indexing | The comment says "Zero-based string index (0 = lowest string)" and the code indexes `Tuning[stringIndex + 1]`, where string 1 is the highest | `Fretboard.GetNote` at `a826864` | The comment is wrong, the code is right | Reproduced, documentation only [2026-09-15](#2026-09-15--a-verdict-on-every-divergence-the-ukulele-and-the-bass-three-appendices) |
| The MCP server's `ga_chord_to_set` spells the chord it was given | It takes its pitch classes from an F# closure that keeps one interval per extension and drops the alterations, so the spelling and the Forte number are wrong | `DomainClosures.fs#L85-L98`, GA's MCP server | `Cm7b5` gives `{C, Eb, G, Bb}` and 4-26, where 4-27 with G♭ is right; `Cdim7` gives 4-27 and "Scale: Major Seventh", where 4-28 is right | Fixed upstream by [#681](https://github.com/GuitarAlchemist/ga/pull/681), merged 2026-09-23. Re-run on `27a1257`: `Cm7b5` gives `{C, Eb, F#, Bb}` and 4-27, `Cdim7` gives 4-28 [2026-09-14](#2026-09-14--gas-mcp-server), [2026-09-24](#2026-09-24--upstream-fixes-checked-again) |
| `ga_set_class_subs` groups the chords it lists under the right quality | Every chord falls under `[maj]`, because the test is `c.EndsWith("")`, which is true of every string | `ChordAtonalTool.cs#L187-L190` | The lists themselves are right; only the grouping is not | Fixed upstream by [#681](https://github.com/GuitarAlchemist/ga/pull/681). Re-run on `27a1257`: the major triads under `[maj]`, the minor ones under `[m]` [2026-09-14](#2026-09-14--gas-mcp-server), [2026-09-24](#2026-09-24--upstream-fixes-checked-again) |
| `ga_scale_by_id` and `ga_scale_by_name` answer for a scale GA knows | `ga_scale_by_id(2741)` returns Major with `Forte Number: Some(n/a)` where 7-35 is expected, and `ga_scale_by_name("Dorian")` is not found at all | GA's MCP server | One wrong field, one missing lookup | Fixed upstream by [#681](https://github.com/GuitarAlchemist/ga/pull/681). Re-run on `27a1257`: `Forte Number: 7-35`, and Dorian found as id 1709 [2026-09-14](#2026-09-14--gas-mcp-server), [2026-09-24](#2026-09-24--upstream-fixes-checked-again) |
| Every MCP tool that takes a chord or a key answers | Several return the string "An error occurred" rather than an MCP error | GA's MCP server | `get_chord_voicings("C", maxFret 3)`, `get_neighboring_keys` and `get_diatonic_chords("A minor")` all answer that way; `ga_search_voicings("C major open chord")` ignored the "open" constraint and returned `8-8-x-x-7-x` | Reproduced; the server's build commit was never confirmed, so these are not pinned to `a826864` [2026-09-14](#2026-09-14--gas-mcp-server). On `27a1257`, `get_neighboring_keys("Key of C")` answers F, C, G; the other three need GaApi running and were not re-run [2026-09-24](#2026-09-24--upstream-fixes-checked-again) |
| `ga_key_from_progression` and `ga_analyze_progression` find the key a cadence establishes | They scored chord roots against the natural minor scale and preferred the first chord's root: `Dm7 G7 Cmaj7` gave D minor, `Am Dm E7 Am` gave A major | [`GuitaristProblemTools.cs#L157-L223`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/GaMcpServer/Tools/GuitaristProblemTools.cs#L157-L223), [`DomainClosures.fs#L255-L336`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Business.DSL/Closures/BuiltinClosures/DomainClosures.fs#L255-L336) | On `27a1257`: `Dm7 G7 Cmaj7` gives C major, ii V I; `Am Dm E7 Am` gives A minor, with E7 labelled `v` and counted outside the key; `Am F C G` still puts A minor first, tied 4/4 with C major, against the tool's own description | Fixed upstream by [#625](https://github.com/GuitarAlchemist/ga/pull/625), merged 2026-09-24, for the two cadences; the numeral and the description remain, not reported [2026-09-15](#2026-09-15--gas-mcp-server-keys-and-progressions), [2026-09-24](#2026-09-24--upstream-fixes-checked-again) |
| `BraceletNotation.tsx` places its dots on its own spokes | The dots are placed at `angle − 90°` while the spokes use `cos(angle)` with no offset, a quarter turn apart; `findSymmetryAxes` also misses the axes that fall between two notes and counts each axis twice | GA's `ReactComponents/ga-react-components` | Read in the source, never run | Reproduced by [#733](https://github.com/GuitarAlchemist/ga/pull/733)'s test, and fixed by it, merged 2026-09-24 [2026-09-15](#2026-09-15--diagrams), [2026-09-24](#2026-09-24--upstream-fixes-checked-again) |

## Experiments

Three questions where a measurement could have gone the other way. The first is the course's own safety net: its theory was written from published definitions, not from GA, so agreeing with GA over all 4,096 sets is evidence about both.

| Question | Hypothesis | Result | Verdict | Where |
|---|---|---|---|---|
| Does the course's from-scratch Rahn prime form agree with GA's minimal-id prime form, over the whole space? | Built from Open Music Theory, Wikipedia and OEIS, independently of GA, so agreement was not guaranteed | They agree for every one of the 4,096 sets. GA's 224 set classes and 352 transposition classes match OEIS A000029 and A000031. Forte's packing differs from Rahn's on 6 set classes and 17 of the 352 transposition classes | Confirmed, with the Forte/Rahn gap quantified separately | [2026-09-15](#2026-09-15--a-verdict-on-every-divergence-the-ukulele-and-the-bass-three-appendices) |
| Does climbing GA's OPTIC ladder one equivalence at a time on a real fingering reach the textbook counts? | Written in advance: it should land on 4096, 352 and 224 | 4096, 352 and 224 | Confirmed | [2026-09-15](#2026-09-15--a-verdict-on-every-divergence-the-ukulele-and-the-bass-three-appendices) |
| Does giving `Diagrams.Fretboard` a tuning parameter change the guitar diagrams it already produced? | Written in advance: it should not — the guitar is just one tuning among the new ones | The fourteen existing SVG files are byte-identical after the change | Confirmed | [2026-09-15](#2026-09-15--a-verdict-on-every-divergence-the-ukulele-and-the-bass-three-appendices) |
...
## 2026-09-24 — Upstream fixes checked again

- GA has merged fixes for the rows of the QA table: [#681](https://github.com/GuitarAlchemist/ga/pull/681) for the MCP tools and [#682](https://github.com/GuitarAlchemist/ga/pull/682) for the parsers on 2026-09-23, [#625](https://github.com/GuitarAlchemist/ga/pull/625) for key detection on 2026-09-24. The course stays pinned to `a826864`, so the lessons and appendices still show the behaviour they were written against.
- [`recheck/probe.cs`](https://github.com/spareilleux/learn/blob/main/code/music-theory-ga/recheck/probe.cs) calls the same tool methods directly, without the MCP transport, on GA [`52f7fd5`](https://github.com/GuitarAlchemist/ga/commit/52f7fd5ce0ba508b2ffd47595e918243c98df5b1); its output is [`recheck/results.txt`](https://github.com/spareilleux/learn/blob/main/code/music-theory-ga/recheck/results.txt). It prints only the facts each finding is about, and [`ga-recheck.yml`](https://github.com/spareilleux/learn/blob/main/.github/workflows/ga-recheck.yml) runs it every Monday against GA's `main` and fails on any difference from `results.txt`: a finding that comes back, or a fix that changes an answer, shows up there. It is not part of `check.sh`, because it needs a full GA clone.
- Fixed, and re-run: `ga_chord_to_set('Cm7b5')` → `{C, Eb, F#, Bb}`, 4-27, and `('Cdim7')` → 0 3 6 9, 4-28 (spelled `{C, Eb, F#, A}`). `ga_set_class_subs('Am')` → the major triads under `[maj]`, the minor ones under `[m]`. `ga_icv_neighbors('C', 1)` → "No other set class within distance 1 of C" instead of twelve identical lines. `ga_scale_by_id(2741)` → `Forte Number: 7-35`, and `ga_scale_by_name('Dorian')` → id 1709. `get_neighboring_keys('Key of C')` → F, C, G.
- Key detection now goes through `KeyIdentificationService`, which adds a cadence weight to the count of diatonic chords ([lines 171-237](https://github.com/GuitarAlchemist/ga/blob/27a1257f7fe5478c4b1bcb6b86ebe101707ac935/Common/GA.Domain.Services/Tonal/KeyIdentificationService.cs#L171-L237)). `ga_analyze_progression("Dm7 G7 Cmaj7")` → "Key: C major", ii V I, and `ga_key_from_progression` agrees; `ga_analyze_progression("Am Dm E7 Am")` → "Key: A minor", i iv v i.
- [#729](https://github.com/GuitarAlchemist/ga/pull/729), merged 2026-09-24, fixes what that re-run still got wrong. In a minor key the major dominant now belongs to the key: `"Am Dm E7 Am"` → i iv V i, confidence 4/4 instead of 3/4. The numeral follows the chord's quality (`Bm7b5` → viiø, `Bdim` → vii°). A tie between relative keys, which share their chords, goes to the key whose tonic triad opens the progression instead of the first in alphabetical order: `[Am, F, C, G]` → A minor, `[C, G, Am, F]` → C major, and `ga_analyze_progression` and `ga_key_from_progression` now give the same key. Re-run on [`52f7fd5`](https://github.com/GuitarAlchemist/ga/commit/52f7fd5ce0ba508b2ffd47595e918243c98df5b1): lines 12-19 of `results.txt`.
- Still not right, not reported: E7 is labelled `v`, because the numeral comes from the natural-minor degree and not from the chord ([`romanFor`](https://github.com/GuitarAlchemist/ga/blob/27a1257f7fe5478c4b1bcb6b86ebe101707ac935/Common/GA.Business.DSL/Closures/BuiltinClosures/DomainClosures.fs#L351-L355)); it also counts outside A minor (3/4), by design in #625, which credits a harmonic-minor V7 through the cadence instead. `ga_key_from_progression(["Am","F","C","G"])` still puts A minor first, tied 4/4 with C major, and the tool's description still promises C major ([line 168](https://github.com/GuitarAlchemist/ga/blob/27a1257f7fe5478c4b1bcb6b86ebe101707ac935/GaMcpServer/Tools/GuitaristProblemTools.cs#L168)).
- Not re-run: `get_chord_voicings`, `get_diatonic_chords("A minor")` and `ga_search_voicings`, which need GaApi running.
- The `BraceletNotation` row was a code reading; [#733](https://github.com/GuitarAlchemist/ga/pull/733), merged 2026-09-24, reproduced it and fixed it. Its test compares the SVG coordinates. On the old code, 3 of its 4 cases fail: C's spoke ends at x=160 while its dot is at x=100; a diminished seventh gives the axis directions [0, 0, 90] instead of [0, 45, 90, 135]; the major scale draws its one axis twice. The spokes now use the dots' `angleToCoordinates`, and the axes are searched in half steps, so the ones between two notes are found. The rendering in a browser is still *to verify*.
- [#734](https://github.com/GuitarAlchemist/ga/pull/734), merged 2026-09-24, fixes the two rows outside the nineteen defects, each with a test that fails on the old code. `getAllInstruments()` returns 122 instruments and 280 tunings where it returned 1 and 2. Nine malformed tunings were restored from `Tunings.txt`, the file the YAML came from: the two pedal steel entries, the Saz, the Huapanguera, the Dulcimer, and the four ukulele tunings that began with `C6` or `D6`. The 5-string banjo tunings now number string 1 from the treble side; tunings that rise or fall all the way are numbered as before. Instrument and tuning names are now the YAML keys, `DropD` rather than `Drop D`.
- One reading in [appendix B](../appendix-instruments/) was wrong. `Dulcimer.LydianMode` is `Bb C4 C4 F3 Bb2`, and I took the first `Bb` for a pitch missing its octave. [`Tunings.txt`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Business.Config/Tunings.txt#L112) reads `Ducimer - Lydian Mode Bb C4 C4 F3 Bb2`: the `Bb` is the end of the tuning's name, *Lydian Mode Bb*, which the conversion to YAML left in the pitch list. The appendix and the QA row now say so.
