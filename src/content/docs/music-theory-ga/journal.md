...
|---|---|---|---|---|
| `getAllInstruments()` returns the instruments the YAML holds | It returns one, and two tunings, falling back to hard-coded guitar values without an error or a log line | `InstrumentsYaml`/`InstrumentsConfig`, `GA.Business.Config` at `a826864` | The file holds 122 instruments and 280 tunings; the loader returned 1 and 2. Seven of the 280 are not pitches at all: two pedal steel entries begin with their tuning's name, one with the word `Tuning`, one with the instrument's name, one is missing an octave | Reproduced, outside the nineteen defects, not reported [2026-09-15](#2026-09-15--a-verdict-on-every-divergence-the-ukulele-and-the-bass-three-appendices) |
| `Tuning.BuildPitchArray` numbers the strings of any tuning correctly | It decides which end is string 1 by comparing the first pitch with the last one only, so a re-entrant tuning comes out backwards | `Tuning.BuildPitchArray` at `a826864` | GA's own two 5-string banjo tunings, whose drone string is written first and sounds higher than the last, are numbered the wrong way round — contradicting `Str`'s own comment that string 1 is the highest | Reproduced, outside the nineteen defects, not reported [2026-09-15](#2026-09-15--a-verdict-on-every-divergence-the-ukulele-and-the-bass-three-appendices) |
