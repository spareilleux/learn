...
The diagram draws frets 0 to 12 in standard tuning, string 1 at the top as in tablature, and marks every position of pitch class C with its pitch. There are six: C3 on string 6 at fret 8 and on string 5 at fret 3, C4 on string 4 at fret 10, string 3 at fret 5 and string 2 at fret 1, and C5 on string 1 at fret 8. Three positions play the very same pitch, C4.

<FretboardC role="img" aria-label="Guitar fretboard in standard tuning, frets 0 to 12, string 1 at the top. Pitch class C: string 6 fret 8 (C3), string 5 fret 3 (C3), string 4 fret 10 (C4), string 3 fret 5 (C4), string 2 fret 1 (C4), string 1 fret 8 (C5)." />
...
- [`Note.Flat.TryParse`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Domain.Core/Primitives/Notes/Note.cs#L219-L237) upper-cases its input *before* replacing `♭` with `b`, then treats a trailing `B` as a flat sign. Those two lines break it in both directions: `"B"` itself becomes B♭, and a real `"D♭"` is rejected, because the replacement leaves a lowercase `b` where the case-sensitive `EndsWith("B")` no longer recognises it.
- [`PitchClass.TryParse`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Domain.Core/Theory/Atonal/PitchClass.cs#L252-L267) reads `A` and `B` as the digits 10 and 11, on purpose: the comment calls them "common hex-like aliases sometimes used in set notation". The convention is real; the bug is the order. The aliases are tried *before* the note names, so `"A"` is pitch class 10 and the note A, pitch class 9, can never be parsed at all.
- [`SimpleIntervalSize.TryParse`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Domain.Core/Primitives/Intervals/SimpleIntervalSize.cs#L155-L164) throws on bad input instead of returning `false`, unlike the [`IParsable<TSelf>.TryParse`](https://learn.microsoft.com/dotnet/api/system.iparsable-1.tryparse) contract, and [`CompoundIntervalSize.TryParse`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Domain.Core/Primitives/Intervals/CompoundIntervalSize.cs#L101-L110) does the same. The table shortens the name: the type is `SimpleIntervalSize`.
