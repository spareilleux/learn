...
You start it from an **image**, a read-only template that you download from a registry (Docker Hub, for example) or build yourself.
...
That is exactly what **WSL 2** provides: a lightweight VM, managed by Windows, with a real Linux kernel.

## Three ways to get Linux containers on Windows

| Approach | Who manages the Linux VM | Tool |
|---|---|---|
| Docker Desktop | Docker, via its own WSL distro `docker-desktop` | `docker` |
| Podman | Podman, via its WSL distro `podman-machine-default` | `podman` |
...
2. **The WSL container API** — a NuGet package (`Microsoft.WSL.Containers`) that lets a **Windows application** use Linux containers in its own logic (C#, and C++/WinRT in preview).
...
On my machine: `docker-desktop` (Docker Desktop) and `podman-machine-default` (Podman). These are "technical" distros managed by those tools, not distros meant to be used directly.
