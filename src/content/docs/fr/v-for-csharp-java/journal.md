...
V 0.5.2 au tag `7647ce1`. Plusieurs lignes opposent la documentation à la version installée, et l'une d'elles est un compilateur qui envoie votre source à un serveur tant qu'on ne connaît pas la variable d'environnement. Rien n'a été signalé : le journal le dit franchement, et précise que le tracker n'a même jamais été fouillé.

Il n'y a pas de tableau d'expériences : aucune entrée de ce journal ne pose d'hypothèse avant la mesure qui l'éprouve. Les deux comparaisons construites — les trois régimes de mémoire, et vingt exécutions répétées d'une course de données — sont des mesures sans a priori consigné, et elles restent dans leurs entrées datées.

| Attendu | Ce qui se passe | Où | Mesure | État |
|---|---|---|---|---|
| Le compilateur n'envoie rien nulle part | V soumet par défaut les erreurs du compilateur C à un serveur. Le rapport emporte le C généré fautif et les lignes de source V, avec les chemins de la machine de l'auteur | `vlib/v/builder/c_error_report.v` 485-506, V 0.5.2 | `Sent C compiler bug report to https://bugs.vlang.io/bug-report.`, avec un identifiant de rapport et une commande `curl -X DELETE` pour l'effacer. Il envoie sauf si `V_C_ERROR_BUG_REPORT_DISABLED` est posé, et s'abstient dans la CI GitHub | Reproduit. La seule mention trouvée est dans `v help build-c`, pas dans le `docs.md` de la 0.5.2. `check.sh` et les exécutions locales posent désormais la variable [2026-09-14](#2026-09-14--surprises-en-écrivant-les-leçons-5-8) |
| `-autofree` libère l'essentiel et le ramasse-miettes fait le reste, comme le dit la documentation | `-autofree` éteint complètement le ramasse-miettes : ce qu'autofree rate fuit donc | `vlib/v/pref/pref.v` 807-811 | `gc_is_enabled()` rend `false` | Reproduit ; la documentation et le code se contredisent [2026-09-14](#2026-09-14--surprises-en-écrivant-les-leçons-1-4) |
| `-prod` change l'optimisation, pas la gestion mémoire | `-prod` avec MSVC sous Windows et sans option `-gc` bascule en `no_gc`, règle que l'auteur n'a trouvée documentée nulle part | `vlib/v/pref/default.v` 369-381 | Après 50 tours : 20 Mo avec `v run`, 542 Mo avec `v -prod run`, 30 Mo avec `v -prod -gc boehm run` | Reproduit ; l'exécuteur Windows de la CI utilise `gcc` pour `-prod` et ne le montre donc jamais [2026-09-14](#2026-09-14--surprises-en-écrivant-les-leçons-1-4) |
| Une fonction anonyme dans un `lock` compile et s'exécute | V l'accepte, puis TCC échoue : le C généré de la fonction anonyme contient le déverrouillage du bloc englobant | génération de code, V 0.5.2 | `'usage' undeclared`, venu de `sync__RwMutex_runlock(&usage->mtx);return _t1;`. Se produit avec `lock` comme avec `rlock`, avec ou sans capture | Reproduit ; épinglé en CI sous `compiler_bugs/b07_return_in_lock.v` pour qu'un correctif se remarque. Le tracker de V n'a jamais été fouillé, rien n'a été signalé [2026-09-14](#2026-09-14--surprises-en-écrivant-les-leçons-5-8) |
| Pousser dans un canal fermé panique, comme le dit la documentation | `ch <- x` sur un canal fermé ne fait rien : le `pushval` généré appelle `try_push_priv` et ignore le `.closed` rendu | `vlib/v/gen/c/cgen.v` 2492-2501 | `popval` rend de même une valeur nulle sur un canal fermé et vide. Avec `or`, les deux signalent `channel closed` | Reproduit, non signalé [2026-09-14](#2026-09-14--surprises-en-écrivant-les-leçons-5-8) |
| Accéder à un champ que n'a qu'une variante d'un type somme est une erreur de compilation | Cela compile en conversion implicite et panique à l'exécution | `vlib/v/checker/checker.v` 3180-3201 | `as cast: cannot cast main.Floating to main.Release` | Reproduit ; vérifié en CI par `panics/p06_variant_field.v` [2026-09-14](#2026-09-14--surprises-en-écrivant-les-leçons-5-8) |
| Un littéral flottant divisé par un entier devient `f64` | `7.0 / 2` garde le type `float literal` | `vlib/v/ast/types.v` 1278 | `typeof(y).name` affiche `float literal` là où `z := 3.5` donne `f64` ; `println(y)` affiche pourtant `3.5` | Reproduit [2026-09-14](#2026-09-14--surprises-en-écrivant-les-leçons-1-4) |
| Convertir une chaîne en nombre signale l'échec | Cela n'échoue jamais | `string.int()`, V 0.5.2 | `'12abc'.int()` vaut 12, `'abc'.int()` vaut 0, `'99999999999'.int()` vaut 2147483647. `strconv.atoi` signale les trois | Par conception, et en silence [2026-09-14](#2026-09-14--surprises-en-écrivant-les-leçons-1-4) |
| `v new` sans entrée s'arrête, ou prend ses valeurs par défaut | Il écrit `'<EOF>'` dans `v.mod` comme description, version et licence | `vlib/os/os.v` 436-441 avec `cmd/tools/vcreate/vcreate.v` 170-181 | `v new hello < /dev/null`. L'outil ne remplace par son défaut qu'une réponse *vide*, et `'<EOF>'` n'est pas vide | Reproduit ; la CI contourne en envoyant trois réponses [2026-09-14](#2026-09-14--installation-de-v-052) |
| Aliaser un tableau et le trancher se comportent pareil | `alias := original` partage en silence la mémoire d'un tableau `mut` ; `first := lines[..2]` sur le même tableau est cloné, avec un avis | `vlib/v/checker/assign.v` 845-855 | Le vérificateur ne cherche que les tranches | Reproduit ; la leçon 4 montre les deux [2026-09-14](#2026-09-14--surprises-en-écrivant-les-leçons-1-4) |
| Deux fonctions de même nom signalent toujours la redéfinition | L'erreur dépend du site d'appel | ordre entre vérificateur et constructeur, V 0.5.2 | `describe('Mission')` donne `builder error: redefinition of function describe` ; `describe(474)` ne donne que `cannot use int literal as string in argument 1 to describe` | Reproduit [2026-09-14](#2026-09-14--surprises-en-écrivant-les-leçons-1-4) |
| `fn [mut x]` capture par référence | `spawn fn [mut total] (n int) { total += n }(i)` dans une boucle compile et laisse `total` à 0 | capture de closure, V 0.5.2 | Le seul indice est `warning: unused variable: total` sur la liste de capture | Reproduit [2026-09-14](#2026-09-14--surprises-en-écrivant-les-leçons-5-8) |
| `v test` affiche la même chose en local et en CI | Avec `CI` ou `GITHUB_JOB` posé, il masque les lignes `OK` sauf si `VTEST_HIDE_OK=0` | `cmd/tools/modules/testing/common.v` 37-43 | Le premier passage en CI de la leçon 8 a échoué sur les trois OS à cause de cela | Reproduit [2026-09-14](#2026-09-14--la-ci) |
| Les nombres d'un rapport de test décrivent les données | `Left value (len: 6): [9, 5]` donne la longueur du texte affiché, et le résumé compte des assertions, pas des fonctions de test | `v test -stats`, V 0.5.2 | `3 failed, 3 passed, 6 total` pour trois fonctions. Une fonction marquée `@[assert_continues]` dont les assertions ont échoué est listée `OK` | Reproduit ; le code de sortie, lui, est juste [2026-09-14](#2026-09-14--surprises-en-écrivant-les-leçons-5-8) |
| `v doctor` indique le compilateur C qui sera employé | Pas toujours | `v doctor` contre `v -showcc`, V 0.5.2 | `v doctor` dit `msvc version N/A` tandis que `v -showcc -prod` compile avec `cl.exe`. Sous macOS, `v doctor` liste TCC alors que `v -showcc` montre `cc` | Reproduit [2026-09-14](#2026-09-14--installation-de-v-052) |
...
- [docs.vlang.io](https://docs.vlang.io/introduction.html) suit `master`, pas la release. Sa page [The default compiler](https://docs.vlang.io/the-default-compiler.html) n'est pas dans [le `docs.md` de 0.5.2](https://github.com/vlang/v/blob/0.5.2/doc/docs.md), et elle n'est même pas d'accord avec `master` : la page web (générée depuis le commit `5db92f0`) dit que l'exécutable `v` "contains only the experimental V3 C compiler", alors que [`doc/docs.md` sur `master`](https://github.com/vlang/v/blob/7cbb67bec0a01138371f3a952f63cf8076fc1b1f/doc/docs.md) dit désormais qu'il "contains the default compiler whose source lives in `vlib/v`". Les leçons vérifient chaque comportement avec 0.5.2 elle-même.
