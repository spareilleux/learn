...
|---|---|---|---|---|
| `getAllInstruments()` rend les instruments que contient le YAML | Il en rend un, et deux accordages, en retombant sans erreur ni ligne de journal sur des valeurs de guitare écrites en dur | `InstrumentsYaml`/`InstrumentsConfig`, `GA.Business.Config` à `a826864` | Le fichier contient 122 instruments et 280 accordages ; le chargeur en a rendu 1 et 2. Sept des 280 ne sont même pas des hauteurs : deux entrées de pedal steel commencent par le nom de leur accordage, une par le mot `Tuning`, une par le nom de l'instrument, une garde un bout de son nom | Corrigé en amont par [#734](https://github.com/GuitarAlchemist/ga/pull/734), fusionnée le 2026-09-24 : le fichier est lu comme un dictionnaire, 122 instruments et 280 accordages, et un échec le dit maintenant sur stderr au lieu de répondre en silence avec la guitare intégrée. Neuf accordages ont été rétablis d'après `Tunings.txt` ; les deux entrées de harpe-guitare gardent leur `|` [2026-09-15](#2026-09-15--un-verdict-sur-chaque-divergence-le-ukulélé-et-la-basse-trois-annexes), [2026-09-24](#2026-09-24--correctifs-amont-vérifiés-à-nouveau) |
| `Tuning.BuildPitchArray` numérote correctement les cordes de n'importe quel accordage | Il décide quel bout est la corde 1 en comparant seulement la première hauteur à la dernière : un accordage rentrant sort donc à l'envers | `Tuning.BuildPitchArray` à `a826864` | Les deux accordages de banjo 5 cordes de GA lui-même, dont la chanterelle est écrite en premier et sonne plus haut que la dernière, sont numérotés à l'envers — ce que contredit le commentaire de `Str`, qui dit que la corde 1 est la plus aiguë | Corrigé en amont par [#734](https://github.com/GuitarAlchemist/ga/pull/734) : le sens est décidé en comptant les pas montants et descendants entre cordes voisines, si bien que les deux accordages de banjo numérotent la corde 1 comme la plus aiguë [2026-09-15](#2026-09-15--un-verdict-sur-chaque-divergence-le-ukulélé-et-la-basse-trois-annexes), [2026-09-24](#2026-09-24--correctifs-amont-vérifiés-à-nouveau) |
...
- [`recheck/probe.cs`](https://github.com/spareilleux/learn/blob/main/code/music-theory-ga/recheck/probe.cs) appelle directement les mêmes méthodes d'outils, sans le transport MCP, sur GA [`52f7fd5`](https://github.com/GuitarAlchemist/ga/commit/52f7fd5ce0ba508b2ffd47595e918243c98df5b1) ; sa sortie est [`recheck/results.txt`](https://github.com/spareilleux/learn/blob/main/code/music-theory-ga/recheck/results.txt). Il n'imprime que les faits dont parle chaque constat, et [`ga-recheck.yml`](https://github.com/spareilleux/learn/blob/main/.github/workflows/ga-recheck.yml) le lance chaque lundi contre la branche `main` de GA et échoue à la moindre différence avec `results.txt` : un constat qui revient, ou un correctif qui change une réponse, s'y voit. Il ne fait pas partie de `check.sh`, parce qu'il lui faut un clone complet de GA.
...
- [#729](https://github.com/GuitarAlchemist/ga/pull/729), fusionnée le 2026-09-24, corrige ce que cette relance donnait encore faux. En mineur, la dominante majeure appartient désormais à la tonalité : `"Am Dm E7 Am"` → i iv V i, confiance 4/4 au lieu de 3/4. Le chiffrage suit la qualité de l'accord (`Bm7b5` → viiø, `Bdim` → vii°). Une égalité entre tonalités relatives, qui partagent leurs accords, va à la tonalité dont l'accord de tonique ouvre la progression, et non plus à la première dans l'ordre alphabétique : `[Am, F, C, G]` → A mineur, `[C, G, Am, F]` → C majeur, et `ga_analyze_progression` et `ga_key_from_progression` donnent maintenant la même tonalité. Relancé sur [`52f7fd5`](https://github.com/GuitarAlchemist/ga/commit/52f7fd5ce0ba508b2ffd47595e918243c98df5b1) : lignes 12-19 de `results.txt`.
...
- La ligne `BraceletNotation` était une lecture de code ; [#733](https://github.com/GuitarAlchemist/ga/pull/733), fusionnée le 2026-09-24, l'a reproduite et corrigée. Son test compare les coordonnées SVG. Sur l'ancien code, 3 de ses 4 cas échouent : le rayon de C finit à x=160 alors que son point est à x=100 ; une septième diminuée donne les directions d'axes [0, 0, 90] au lieu de [0, 45, 90, 135] ; la gamme majeure trace deux fois son unique axe. Les rayons utilisent maintenant le même `angleToCoordinates` que les points, et les axes sont cherchés par demi-pas, si bien que ceux qui tombent entre deux notes sont trouvés. Le rendu dans un navigateur reste *à vérifier*.
- [#734](https://github.com/GuitarAlchemist/ga/pull/734), fusionnée le 2026-09-24, corrige les deux lignes hors des dix-neuf défauts, chacune avec un test qui échoue sur l'ancien code. `getAllInstruments()` rend 122 instruments et 280 accordages là où il en rendait 1 et 2. Neuf accordages mal formés ont été rétablis d'après `Tunings.txt`, le fichier dont vient le YAML : les deux entrées de pedal steel, le saz, la huapanguera, le dulcimer et les quatre accordages de ukulélé qui commençaient par `C6` ou `D6`. Les accordages de banjo à 5 cordes numérotent maintenant la corde 1 du côté aigu ; ceux qui montent ou descendent d'un bout à l'autre sont numérotés comme avant. Les noms d'instruments et d'accordages sont maintenant les clés du YAML, `DropD` plutôt que `Drop D`.
- Une lecture de l'[annexe B](../appendix-instruments/) était fausse. `Dulcimer.LydianMode`, c'est `Bb C4 C4 F3 Bb2`, et j'avais pris le premier `Bb` pour une hauteur sans octave. [`Tunings.txt`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Business.Config/Tunings.txt#L112) donne `Ducimer - Lydian Mode Bb C4 C4 F3 Bb2` : le `Bb` est la fin du nom de l'accordage, *Lydian Mode Bb*, que la conversion en YAML a laissée dans la liste des hauteurs. L'annexe et la ligne QA le disent maintenant.
