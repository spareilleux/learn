...
<iframe src="../../../threejs-demos/01-scene.html" title="Leçon 1, en direct" loading="lazy" width="100%" height="420" allow="fullscreen; autoplay; xr-spatial-tracking" style={{ border: 0, display: "block", height: "min(420px, 70vh)" }}></iframe>

</details>

[Ouvrir en plein écran](../../../threejs-demos/01-scene.html). **À essayer** : le curseur *cubes* du panneau recharge la page avec `?cubes=` : mettez-le à 3 pour la scène de l'exercice 1. Ouverte en plein écran, la page affiche dans le titre de l'onglet le backend qu'elle a choisi, la réponse à l'exercice 3.
...
L'[expérience 15, un GPU faible](../14-guitar-alchemist-lab/#15-un-gpu-faible), dans la leçon 14, rend un manche sur SwiftShader : avec le pixel ratio de 3 de GA, une image prend 208 ms, et un contrôleur qui descend le pixel ratio à 0.655 la ramène à 60 images par seconde. Elle montre aussi que `powerPreference` ne peut pas choisir le GPU sous Windows.
