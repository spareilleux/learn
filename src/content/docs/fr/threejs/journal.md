...
three.js r186 et les bibliothèques autour sont le logiciel de quelqu'un d'autre ; ce tableau retient ce que les leçons y ont trouvé, pas ce qu'elles ont trouvé dans les pages du cours. L'essentiel vient du passage à `WebGPURenderer`, assez récent pour que ce soit souvent l'habitude WebGL qui ait tort. Rien n'a été signalé en amont, sauf là où la colonne État nomme un ticket existant.

| Attendu | Ce qui se passe | Où | Mesure | État |
|---|---|---|---|---|
| `WebGPURenderer` compte les appels de dessin de la scène, comme `WebGLRenderer` | Avec un tone mapping, ou un espace colorimétrique de sortie différent de l'espace de travail, il rend dans une cible puis la dessine dans une passe de sortie | three.js r186 `WebGPURenderer` | 2 appels de dessin et 13 triangles pour un cube, contre 1 sur WebGL ; les deux objets `info` n'ont d'ailleurs pas la même forme | Reproduit [2026-09-16](#2026-09-16--surprises) |
| `@types/three` décrit `toJSON()` tel que le runtime l'écrit | En 0.186.0, `Object3DJSON` déclare `children` comme des chaînes et n'a ni `geometries` ni `materials` ; le runtime écrit des objets et les deux tableaux | `@types/three` 0.186.0 | Les erreurs du compilateur sont conservées dans `errors/l01_tojson_types.ts` | Reproduit. Aucun ticket amont cherché, aucun ouvert [2026-09-16](#2026-09-16--surprises) |
| `PCFSoftShadowMap` fonctionne sur `WebGPURenderer` | Retiré en r186, et il avertit. R3F le pose pour tout `shadows` vrai et récolte le même avertissement à chaque rendu de `<Canvas>` | three.js r186, @react-three/fiber 9.7.0 | `LunarLanderEngine.ts` et `MinimalThreeInstrument.tsx` de GA l'associent encore à `WebGPURenderer` | Reproduit [2026-09-16](#2026-09-16--surprises) · [2026-09-16](#2026-09-16--leçons-9-à-13--ce-quont-trouvé-les-pages) · Pas encore dans GA, qui épingle three 0.180.0, où il fonctionne toujours ; [2026-09-24](#2026-09-24--correctifs-amont) |
| Une lumière ponctuelle et un spot convertissent les lumens en candelas de la même façon | `SpotLight.power` utilise π au lieu de 4π et ne varie pas avec l'angle du cône | three.js `SpotLight.power` | 800 lm donnent 63,66 cd sur une ponctuelle et 254,65 sur un spot | Reproduit [2026-09-16](#2026-09-16--surprises) |
| Construire un environnement équirectangulaire coûte à peu près une passe | Le PMREM coûte 17 rendus : 1 dans le premier niveau, puis 2 pour chacun des 8 autres, même pour une simple texture équirectangulaire | three.js `PMREMGenerator` | 17 rendus pour un environnement de 256 sur 128 | Reproduit, par conception [2026-09-16](#2026-09-16--surprises) |
| Le bundler embarque les décodeurs Draco que le navigateur charge | Depuis r185, `DRACOLoader` les résout avec `import.meta.url` : Vite les copie donc tous les cinq | three.js r185+ `DRACOLoader`, Vite | 1,31 Mo embarqués là où un navigateur en charge deux ; le build avertit aussi que `three.webgpu` dépasse 500 ko | Reproduit, comportement changé depuis r184 [2026-09-16](#2026-09-16--surprises) |
| La compression garde tous les sommets de la géométrie | Draco et meshopt suppriment tous deux les deux sommets polaires que `SphereGeometry` crée et qu'aucun triangle n'utilise ; le `weld()` de Draco fusionne en plus les coins du corps non indexé | Draco, meshopt, glTF Transform 4.5.0 | Les comptes de sommets avant et après, dans `expected/` | Reproduit, défendable [2026-09-16](#2026-09-16--surprises) |
| Le `<Text>` de drei se dessine sur `WebGPURenderer` comme sur WebGL | Il dessine un quad vide : troika-three-text injecte son shader par `onBeforeCompile`, que le renderer à nœuds n'appelle jamais | drei 10.7.8 | Quad vide | Reproduit. Reste ouvert : l'existence d'un troika-three-text prêt pour WebGPU [2026-09-16](#2026-09-16--leçons-9-à-13--ce-quont-trouvé-les-pages) |
| `setColorAt` colore les instances quoi qu'ait fait le premier rendu | Un `InstancedMesh` l'ignore si son premier rendu n'avait pas de couleurs d'instance | three.js `InstancedMesh` | Les marqueurs du manche sont restés blancs jusqu'à ce que l'attribut soit alloué d'avance | Reproduit, contourné [2026-09-16](#2026-09-16--leçons-9-à-13--ce-quont-trouvé-les-pages) |
| WebXR sur `WebGPURenderer` se comporte comme sur `WebGLRenderer` | Il lui faut `XRGPUBinding` et la fonctionnalité `webgpu` ; son message d'erreur nomme un `VRButtonGPU` qui n'existe pas ; `WebGLXRFallback` échange les renderers mais les contrôleurs pris au premier cessent de recevoir des événements | three.js r186, WebXR sur WebGPU | Seul le `WebGLRenderer` classique a dessiné les deux yeux sous IWER | Reproduit [2026-09-16](#2026-09-16--leçons-9-à-13--ce-quont-trouvé-les-pages) |
| `InstancedMesh` fonctionne avec des morph targets | Il échoue avec une seule instance, `morphTargetInfluences` n'étant jamais créé, et autrement avec plusieurs, dans `UniformArrayNode.updateBuffer`. Il lui faut aussi `geometry.morphTargetsRelative = true`, sans le dire | three.js r186 | Deux échecs distincts, un par nombre d'instances | Reproduit [2026-09-17](#2026-09-17--leçon-14-le-labo-3d-guitar-alchemist) |
| `BatchedMesh` regroupe en peu d'appels de dessin sur tous les backends | Sur WebGPU, il dessine un appel par objet | three.js r186 `BatchedMesh`, backend WebGPU | 44 appels pour un manche contre 3 sur WebGL 2, qui utilise `WEBGL_multi_draw` | Reproduit [2026-09-17](#2026-09-17--leçon-14-le-labo-3d-guitar-alchemist) |
| `renderer.info.memory.texturesSize` compte les textures compressées | Il compte 0 octet pour les textures KTX2, et compte les cibles de rendu à la place | three.js r186 `renderer.info.memory` | 0 octet pour le jeu de bois compressé, ce qui a empêché de finir l'expérience 6 | Reproduit. Reste ouvert : mesurer la mémoire GPU KTX2 avec un outil GPU [2026-09-17](#2026-09-17--leçon-14-le-labo-3d-guitar-alchemist) |
| `powerPreference` choisit un GPU | Chromium sous Windows l'ignore et avertit ; les autres OS non | Chromium 153 | `check.sh` filtre la ligne | Connu en amont, ticket Chromium 369219127 [2026-09-16](#2026-09-16--leçons-9-à-13--ce-quont-trouvé-les-pages) |
| L'addon `RapierPhysics` de three.js suit une version courante de Rapier | Celui de r186 charge encore 0.17.3 depuis skypack.dev, alors que le cours utilise 0.20.0 | three.js r186 `examples/jsm/physics/RapierPhysics` | Deux versions mineures d'écart | Reproduit, le cours n'utilise pas l'addon [2026-09-16](#2026-09-16--leçons-9-à-13--ce-quont-trouvé-les-pages) |
| Le build WebGPU de R3F n'embarque que le point d'entrée WebGPU | Il importe `three` à côté de `three/webgpu`, donc le cœur WebGL est embarqué aussi | @react-three/fiber 9.7.0 | 565 ko gzip contre 210 ko pour la même scène sans React | Reproduit. Reste ouvert : aliaser `three` vers `three/webgpu` [2026-09-17](#2026-09-17--leçon-14-le-labo-3d-guitar-alchemist) |

## Expériences

La leçon 14 a mené quinze expériences dans le labo 3D Guitar Alchemist. Chaque hypothèse et ses prédictions chiffrées ont été versées le 16 septembre, la veille des mesures : c'est ce qui permet à une ligne ci-dessous de dire *réfutée*. Sur les 61 prédictions, 19 étaient fausses en tout ou partie et 2 n'ont pas pu être mesurées. Mesuré sur une RTX 5080 sous Chromium 153, temps GPU par requêtes d'horodatage WebGPU. Le détail est dans la [leçon 14](#2026-09-17--leçon-14-le-labo-3d-guitar-alchemist).

| Question | Hypothèse | Résultat | Verdict |
|---|---|---|---|
| Quelle façon de dessiner 16 manches de guitare coûte le moins : celle de GA, instanciée, batchée ou fusionnée ? | Les appels de dessin dictent le temps CPU ; le GPU bouge à peine de l'une à l'autre | La version batchée sur WebGPU a tiré 44 appels, et non les 4 ou moins prédits. Les 16 manches de GA coûtent 2,9 ms de CPU, et non les 10 ms ou plus prédites | Majoritairement réfutée |
| Réécrire les sommets d'une corde vibrante sur le CPU contre le faire en TSL, jusqu'à 600 cordes | À 600 cordes, la réécriture CPU coûte davantage, et le coût GPU est comparable | CPU 1,8 ms contre TSL 0,36 ms, un facteur 5, comme prédit. Mais TSL a coûté 1,7 fois le temps GPU de la version CPU | Confirmée côté CPU, réfutée côté GPU |
| Désigner une note parmi 10 000 : `Raycaster` contre une passe d'identifiants sur GPU | La sélection GPU l'emporte à 10 000 objets | `Raycaster` 0,465 ms contre 3,395 ms pour la sélection GPU | Réfutée |
| Transposer 49 152 bracelets de classes de hauteurs, sur CPU contre TSL | Le CPU paie la reconstruction de 49 152 matrices par image : 4 ms ou plus | CPU 2,455 ms, en dessous de la prédiction. TSL 0,29 ms, comme prédit | Confirmée pour TSL, réfutée pour le CPU |
| Un nuage de points OPTIC-K jusqu'à 1 000 000 de points, et si le calcul TSL exige WebGPU | Les points restent bon marché jusqu'au million ; le compute shader ne tourne pas sur WebGL 2 | Les points ont tenu. Le compute shader a bien tourné sur WebGL 2, par transform feedback, à 2,8 ms GPU | Confirmée pour les points, réfutée pour le backend |
| Ce que coûtent les textures de bois non compressées, et ce que KTX2 économise | RGBA8 coûte environ 22 Mo ; KTX2 en BC7 en coûte environ le quart | 21,33 Mo confirmés. Le chiffre KTX2 n'a pas pu être lu : `renderer.info` compte 0 pour les textures compressées | Moitié confirmée, moitié non mesurable |
| Une grille de matériaux PBR : combien de programmes de shader, et ce qu'ajoute `MeshPhysicalMaterial` | Un programme par type de matériau, partagé sur la grille ; le matériau physique coûte plus de temps GPU | 16 programmes dans les deux cas, comme prédit. Le matériau physique a coûté 1,46 fois le temps GPU | Confirmée |
| Ce que coûte une shadow map dynamique face à une cuite | Une shadow map dynamique double à peu près les appels de dessin | 13 appels contre 8, un facteur 1,6. La cuisson a pris 398 ms, comme prédit | Confirmée en gros |
| Ce que coûte le bloom en 1080p | Entre 0,5 et 1,5 ms GPU | 0,135 ms, quatre fois sous la prédiction la plus basse | Réfutée |
| Un mur de 1 000 guitares avec LOD et frustum culling | Le LOD divise les triangles par environ 10 | Par 11,2 | Confirmée |
| Si le CCD de Rapier empêche un médiator cinématique de traverser une corde | Le CCD empêche la traversée | Le médiator a traversé à 30 comme à 60 pas par seconde, CCD activé ou non | Réfutée |
| Si un manche est à l'échelle 1:1 en WebXR, sous l'émulation IWER | Du sillet à la douzième case, on mesure 0,324 m | 0,324 m | Confirmée |
| De combien il faut programmer une note à l'avance pour que WebAudio et l'image s'accordent | L'avance naïve vaut à peu près la somme des latences, 50 ms, à une image près | 31,8 ms, soit 18 ms de moins que la somme | Réfutée |
| Ce que coûte React Three Fiber face à three.js nu, pour la même scène | Peu cher par image, cher au démarrage (+30 à 150 ms) et dans le bundle (+50 à 90 ko) | Peu cher par image, comme prédit. Première image +430 ms et bundle +195 ko, tous deux très au-dessus | Confirmée par image, réfutée sur les deux coûts |
| Si un ratio de pixels adaptatif se stabilise sur un GPU faible | Le temps GPU est proportionnel au nombre de pixels, et le régulateur se stabilise en moins de 60 images | Il s'est stabilisé à 0,655 en moins de 60 images | Confirmée |
...
- La ligne `PCFSoftShadowMap` ne s'applique pas encore à GA. Le retrait et son avertissement sont réels en r186 (`Renderer.js` et `ShadowNode.js` du paquet `three@0.186.0`), mais GA épingle three 0.180.0 et @react-three/fiber 8.18.0. En 0.180, `ShadowNode` associe encore `PCFSoftShadowMap` à son filtre doux : pas d'avertissement, et les ombres douces fonctionnent.
- Les deux associations de GA avec `WebGPURenderer`, dans `LunarLanderEngine.ts` et `MinimalThreeInstrument.tsx`, commenceront à avertir quand GA passera à r186 ou plus. Les changer maintenant ferait perdre les ombres douces : [#733](https://github.com/GuitarAlchemist/ga/pull/733) les laisse pour cette mise à jour et le dit. Lu dans les sources du paquet, pas exécuté (*à vérifier*).
