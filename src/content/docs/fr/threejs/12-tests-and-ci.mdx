...
La leçon 14 applique la règle de cette leçon aux temps : sa CI ne compare que les compteurs de `renderer.info`, sur trois OS, et jamais un temps. [Comment ses mesures ont été faites](../14-guitar-alchemist-lab/#comment-les-mesures-ont-été-faites) explique le reste : 240 images chronométrées sans `requestAnimationFrame` ni vsync, et les temps GPU tirés des requêtes d'horodatage de WebGPU.
