...
| Une issue dont le travail est livré est fermée | L'issue 108 était ouverte alors que sa porte de contrôle était sur `main` ; le worker de l'usine n'a rien modifié | [`tests/hexagonal-direction.test.mjs`](https://github.com/GuitarAlchemist/gaia/blob/8ed4dfca865696f6b54cb37ce077b0d10ece35df/tests/hexagonal-direction.test.mjs) | 0 modification, 1 exécution de l'usine dépensée | Fermée comme terminée, [gaia#108](https://github.com/GuitarAlchemist/gaia/issues/108) |
| Un job de l'usine se termine après son worker | Le job de l'issue 108 était encore `STARTED` une dizaine d'heures après la fin de son worker, occupant l'unique place de l'hôte | `C:/Gaia/state`, `portfolio:autonomous status` | Worker terminé à 00:25 heure locale, job `STARTED` à 10:00 | Expliqué : `watch` s'arrête sur `RECONCILIATION_REQUIRED` ([`scripts/github-portfolio-autonomous.mjs:207`](https://github.com/GuitarAlchemist/gaia/blob/8ed4dfca865696f6b54cb37ce077b0d10ece35df/scripts/github-portfolio-autonomous.mjs#L207)) ; job retiré avec `retire-closed` [2026-09-26](#2026-09-26--le-candidat-de-lusine-et-les-tests-que-personne-na-lancés) |
| Un candidat marqué `CANDIDATE_READY` et approuvé a passé ses tests | Aucun test n'est jamais exécuté : les outils du worker excluent un shell, le relecteur est en lecture seule, et le « superviseur » qui, selon le prompt, lance les tests n'existe pas dans `src/` | [`src/factory-visible-claude.mjs:354`](https://github.com/GuitarAlchemist/gaia/blob/8ed4dfca865696f6b54cb37ce077b0d10ece35df/src/factory-visible-claude.mjs#L354) | Candidat approuvé de l'issue 104 : 16/17 de ses propres tests, décompte du README non mis à jour | Signalé dans [gaia#163](https://github.com/GuitarAlchemist/gaia/issues/163) ; candidat corrigé à la main [2026-09-26](#2026-09-26--le-candidat-de-lusine-et-les-tests-que-personne-na-lancés) |
...
## 2026-09-26 — Le candidat de l'usine, et les tests que personne n'a lancés

- **Pourquoi `watch` s'était tu.** Il ne s'était pas figé, il s'était arrêté. Après un worker sans modification sur l'issue 108, le job a fini en `RECONCILIATION_REQUIRED`, et `watch` s'arrête sur ce statut par conception ([`scripts/github-portfolio-autonomous.mjs:207`](https://github.com/GuitarAlchemist/gaia/blob/8ed4dfca865696f6b54cb37ce077b0d10ece35df/scripts/github-portfolio-autonomous.mjs#L207)). Le job occupait alors l'unique place. Comme l'issue et sa Draft étaient fermées, `retire-closed` l'a retiré en `ABANDONED` après un aperçu. J'ai fait la sauvegarde de la base que demande la documentation après coup au lieu d'avant ; rien n'a été perdu.
- **L'issue 104 a produit un vrai candidat :** `CANDIDATE_READY`, relecteur `APPROVE`, trois fichiers (un manifeste pur de reprise de voie, son document et 17 tests). Il est resté local : l'usine ne publie jamais.
- **L'approbation ne voulait pas dire que les tests passent.** Lancé avec la Node épinglée 26.8.1, le candidat échouait à l'un de ses propres tests et au test du décompte du README. La cause est structurelle. Le prompt du worker dit « run the narrowest relevant tests », mais ses outils sont `Read,Write,Edit,Glob,Grep` ([`src/factory-visible-claude.mjs:354`](https://github.com/GuitarAlchemist/gaia/blob/8ed4dfca865696f6b54cb37ce077b0d10ece35df/src/factory-visible-claude.mjs#L354)), et le prompt ajoute que « tests are run separately by the supervisor ». Aucun code de `src/` ne les lance. Signalé dans [gaia#163](https://github.com/GuitarAlchemist/gaia/issues/163), qui propose une étape de vérification déterministe, tenue par l'hôte, avant la relecture, plutôt qu'un shell pour le modèle.
- **C'est le test qui était faux, pas le code.** Il ajoutait `src/extra.mjs` après `src/resume-manifest.mjs`. Le module exige des artefacts triés par chemin, donc l'observation était refusée comme malformée avant toute comparaison. Ajouter `tests/extra.mjs` garde l'intention du test ; avec le décompte du README mis à jour, le candidat passe et a été publié sur la Draft [gaia#158](https://github.com/GuitarAlchemist/gaia/pull/158).
