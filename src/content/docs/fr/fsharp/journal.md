...
- `ChordParser.parse` exige maintenant le symbole entier ([#682](https://github.com/GuitarAlchemist/ga/pull/682)) et lit `7sus4`, `Maj7` et `omit` ([#689](https://github.com/GuitarAlchemist/ga/pull/689)), toutes deux fusionnées le 2026-09-23. Relancé sur GA [`27a1257`](https://github.com/GuitarAlchemist/ga/commit/27a1257f7fe5478c4b1bcb6b86ebe101707ac935) par le [`recheck/probe.cs`](https://github.com/spareilleux/learn/blob/main/code/music-theory-ga/recheck/probe.cs) de music-theory-ga : `C7sus4` → `Ok` avec `Components = [Extension "7"; Extension "sus4"]`, et `Am(maj7)` → `Error in Ln: 1 Col: 3`. La leçon 4 reste épinglée à `32f143c` et montre toujours le préfixe accepté.
- `GetNormalForm` est inchangé : son fichier est identique à `32f143c` et à `27a1257`.
- [#734](https://github.com/GuitarAlchemist/ga/pull/734), fusionnée le 2026-09-24, corrige `GetNormalForm` : les égalités d'empan sont départagées comme le fait Rahn, par les intervalles du premier élément à l'avant-dernier, puis à l'antépénultième, et ainsi de suite. `{0, 4, 7, 8}` donne maintenant `[0; 3; 4; 8]`, comme sa transposition par 8. Son test passe les 4 096 ensembles sous T1 à T11 ; sur l'ancien code, 5 920 de ces 45 045 transpositions changeaient la réponse. Le commentaire de documentation n'appelle plus le résultat une forme première de Forte : c'est l'ordre normal transposé à zéro, sans l'inversion. Un dernier test fixe `{0, 1, 5, 6, 8}`, un ensemble sur lequel Rahn et Forte ne sont pas d'accord.
