...
| 5 | Exceptions, `null` et `Optional` *(à venir)* | exceptions, `using`, types référence nullables |
| 6 | Lambdas et interfaces fonctionnelles | délégués, `Func`/`Action`, événements |
| 7 | Collections et Streams | `List<T>`, `Dictionary`, LINQ |
| 8 | Pattern matching | expressions `switch`, motifs `is`, records |
| 9 | Concurrence et threads virtuels | `Task`, `async`/`await`, `lock`, `Parallel` |
