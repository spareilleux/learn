...
OpenJDK 25+36, Maven 3.9.16, Gradle 9.7.1, comparés tout du long au SDK .NET 10. Plusieurs lignes sont des comportements par défaut qui diffèrent de ceux de .NET : le comportement Java y est documenté, et la surprise tient seulement à l'habitude C# qu'on apporte ; la colonne État dit lesquelles. Les deux premières lignes viennent de la construction du plugin JetBrains de GuitarAlchemist/ga, par où commencent les cas pratiques de ce journal. Rien n'a été signalé en amont.

Il n'y a pas de tableau d'expériences : aucune entrée ne pose d'hypothèse avant la mesure qui l'éprouve. La seule phrase qui en a la forme — le premier programme en concurrence aurait perdu plus de 90 % de ses incréments parce que le JIT n'avait pas encore compilé la boucle — explique un nombre déjà consigné, et le journal la marque *à vérifier*.

| Attendu | Ce qui se passe | Où | Mesure | État |
|---|---|---|---|---|
| Gradle nomme le problème quand le JDK est trop récent pour lui | Gradle 8.5 sur JDK 25 échoue sur un message qui n'est que le numéro de version de Java | `jetbrains-plugin/` de GuitarAlchemist/ga à `5560b883`, Gradle 8.5, JDK 25 | `* What went wrong:` suivi de `25` ; la pile montre une `java.lang.IllegalArgumentException: 25` levée pendant que Gradle compile le script de build Kotlin | Reproduit ; monter Gradle déplace l'échec à la ligne suivante [cas pratiques](#cas-pratiques-dans-mes-dépôts) |
| Une version de Gradle qui accepte JDK 25 suffit à construire le plugin | Gradle 9.7.1 va plus loin et meurt dans l'ancien plugin `org.jetbrains.intellij` 1.16.1 | le même build, Gradle 9.7.1 | `class org.jetbrains.intellij.MemoizedProvider overrides final method org.gradle.api.internal.provider.AbstractMinimalProvider.toString()` | Reproduit ; la migration vers IntelliJ Platform Gradle Plugin 2.x est l'étape suivante, pas faite [cas pratiques](#cas-pratiques-dans-mes-dépôts) |
| Maven prévient quand il résout une dépendance plus ancienne que celle qu'un module a demandée | Il met Commons Lang 3.14.0 dans le class path contre une demande de 3.20.0, en silence, et le programme échoue à l'exécution | Maven 3.9.16, `l10/check.sh` | `NoClassDefFoundError` à l'exécution ; il faut `-Dverbose` rien que pour voir qu'une autre version était demandée. Le `NU1605` de NuGet fait de la même situation une erreur par défaut | Par conception [2026-09-13](#2026-09-13--leçon-10) |
| « Le plus proche gagne » donne à une bibliothèque et à son artefact d'agent des versions assorties | Mockito 5.23.0 demande Byte Buddy 1.17.7, AssertJ 3.27.7 demande 1.18.3, et Maven associe `byte-buddy` 1.18.3 à `byte-buddy-agent` 1.17.7 | `l11`, Maven 3.9.16 | Ça marche, parce que le chemin d'AssertJ est plus court et apporte la version la plus récente — par chance, pas par règle | Reproduit, non signalé [2026-09-13](#2026-09-13--leçon-11) |
| `CompletableFuture.cancel(true)` interrompt la tâche en cours, comme le dit `mayInterruptIfRunning` | Le future se dit annulé pendant que sa tâche va jusqu'au bout | `CompletableFuture`, OpenJDK 25+36 | La Javadoc dit que l'argument « has no effect in this implementation ». Une tâche de cinq secondes annulée a quand même pris cinq secondes, parce que le `close()` de l'exécuteur l'a attendue | Par conception, et documenté ; la leçon 9 le montre désormais avec une tâche d'une seconde [2026-09-13](#2026-09-13--leçon-9) |
| BlockHound s'installe par `BlockHound.install()`, comme le montre son README | Sur JDK 25 l'auto-attachement échoue, le contournement documenté échoue autrement, et seul `-javaagent` marche | BlockHound 1.0.17, JDK 25, `l11` | `Could not self-attach to current VM using external process` ; avec `-Djdk.attach.allowAttachSelf=true`, `Agent JAR loaded but agent failed to initialize` | Reproduit, non signalé ; la leçon 11 passe par `-javaagent` [2026-09-13](#2026-09-13--leçon-11) |
| Une bibliothèque d'instrumentation actuelle ne demande aucune option JVM dépréciée et nomme la méthode appelée | BlockHound exige une option dépréciée depuis JDK 13, et signale `Thread.sleep` sous un nom privé du JDK | BlockHound 1.0.17, JDK 25, `l11/check.sh` | `-XX:+AllowRedefinitionToAddDeleteMethods` ; `Thread.sleep` signalé comme `java.lang.Thread.sleepNanos0` | Reproduit, non signalé [2026-09-13](#2026-09-13--leçon-11) |
| Un processeur d'annotations qui échoue rapporte son erreur | Maven affiche un message générique et cache la cause sauf avec `-e` | NullAway 0.14.1 sur Error Prone 2.50.0, JDK 25, `l11` | `An unknown compilation problem occurred` ; la vraie erreur, une `IllegalAccessError` sur l'API interne de javac, n'apparaît qu'avec `mvn -e` | Reproduit ; un `.mvn/jvm.config` avec des lignes `--add-exports` règle le problème [2026-09-13](#2026-09-13--leçon-11) |
| `javac -XDrawDiagnostics` et l'API `javax.tools` donnent la même clé de diagnostic | Elles divergent sur le même extrait refusé | OpenJDK 25+36, ajout dans une `List<? extends Number>` | La ligne de commande donne `compiler.err.cant.apply.symbols`, l'API `compiler.err.prob.found.req` | Reproduit ; les leçons citent ce que voit l'API, puisque les tests passent par elle [2026-09-13](#2026-09-13--leçons-1-à-4) |
| `YYYY` dans un motif de date est l'année civile, comme en .NET | C'est l'année de la semaine, qui diffère en fin d'année et dépend de la locale | formatage de dates Java, leçon 12 | Le 31 décembre 2026 s'affiche `2027` avec `Locale.US` et `2026` avec `Locale.FRANCE` | Par conception [2026-09-13](#2026-09-13--leçon-12) |
| Le `HttpClient` de Java suit les redirections et expire, comme celui de .NET | Il ne fait ni l'un ni l'autre sans configuration | `java.net.http.HttpClient`, JDK 25 | Du code .NET porté obtient un 302 au corps vide, ou un appel qui peut attendre indéfiniment | Par conception [2026-09-13](#2026-09-13--leçon-12) |
| Surefire affiche `@DisplayName` et attribue les tests à la classe qui les déclare | Il ignore `@DisplayName` sans option de rapporteur, et a attribué les tests de toute une classe à sa classe imbriquée | Maven Surefire avec JUnit 6, `l11` | Les neuf tests de `PriceCalculatorTest` attribués à sa classe imbriquée dans la sortie console | Reproduit, non signalé [2026-09-13](#2026-09-13--leçon-11) |
| Un `Map.of(…)` immuable s'itère dans un ordre stable | L'ordre change d'un démarrage de JVM à l'autre | `Map.of`, OpenJDK 25+36 | Quatre exécutions du même programme d'une ligne ont donné deux ordres | Par conception : `Map.of` tire son ordre d'itération au hasard à chaque JVM. La sortie de l'exemple n'en dépend plus [2026-09-13](#2026-09-13--leçons-5-à-8) |
