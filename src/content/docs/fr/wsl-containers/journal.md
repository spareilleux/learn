...
`wslc`, son SDK C# et Docker Desktop sont tous le logiciel de quelqu'un d'autre, et `wslc` est assez récent pour que l'essentiel de ce qui suit porte sur l'outil plutôt que sur les conteneurs. Deux lignes sont de la documentation qui ne correspond pas à la version livrée. Rien n'a été signalé en amont.

| Attendu | Ce qui se passe | Où | Mesure | État |
|---|---|---|---|---|
| L'extrait `Microsoft.WSL.Containers` de Microsoft Learn compile contre le paquet | Cinq erreurs de compilation : `ComponentFlags`, `SessionSettings.MemoryMB`, `ProcessSettings.CmdLine` et `DeleteContainerFlags` n'existent pas. Le paquet les nomme `IReadOnlyList<Component>`, `MemorySizeInMB`, `CommandLine` et `DeleteContainerOption` | `Microsoft.WSL.Containers` 2.9.9 | Cinq paires de noms, documentation contre paquet | Reproduit, non signalé [2026-09-13](#2026-09-13--un-programme-c-avec-microsoftwslcontainers) |
| Le framework cible du paquet correspond au SDK contre lequel il a été construit | `wslcsdkcs.dll` a été construit contre `Microsoft.Windows.SDK.NET` 10.0.26100.79, qui n'existe pas sur nuget.org — la plus proche est 10.0.26100.80 — d'où un CS1705 à la référence | `Microsoft.WSL.Containers` 2.9.9 | CS1705, et une version de SDK impossible à restaurer | Reproduit, contourné en épinglant `WindowsSdkPackageVersion` [2026-09-13](#2026-09-13--un-programme-c-avec-microsoftwslcontainers) |
| Un conteneur est nettoyé quand `Start()` lève | Il survit dans `storage.vhdx`, et le `Start()` suivant avec le même nom échoue | `Microsoft.WSL.Containers` 2.9.9 | `COMException`, « name already in use » | Reproduit, contourné par un `Delete(Force)` en `finally` et un nettoyage au démarrage [2026-09-13](#2026-09-13--un-programme-c-avec-microsoftwslcontainers) |
| Un conteneur créé par l'API a accès au réseau, comme `wslc run` le lui donne | Sans `ContainerSettings.NetworkingMode`, il obtient `NetworkMode: none` et aucun accès ; il lui faut un `Bridged` explicite | `Microsoft.WSL.Containers`, 2.9.11 | `ip -4 addr` et `wget` avant et après | Reproduit, défaut défendable, non documenté [2026-09-13](#2026-09-13--aller-plus-loin--wslcimage-réseau-kubernetes-interface-graphique) |
| `wslc` a une sous-commande Compose, comme `docker compose` | `Unrecognized command: 'compose'` — il n'y en a pas en 2.9.11 | `wslc` 2.9.11 | `wslc --help` | Reproduit ; le cours écrit son propre lanceur à la place [2026-09-13](#2026-09-13--compose-avec-wslc) |
| `wslc image prune -f` force, comme le `-f` de `docker` | `-f` veut dire `--filter`, et `--force` n'existe pas | CLI `wslc` 2.9.11 | Un drapeau qui se lit comme `--force` et prend un filtre | Reproduit, piège que la leçon conserve [2026-09-13](#2026-09-13--les-dernières-questions-ouvertes) |
| La source d'un montage lié est lue comme un chemin dans le conteneur | Elle est analysée comme un chemin Windows : `-v /var/run/docker.sock:/var/run/docker.sock` crée donc en silence un vrai dossier vide `C:\var\run\docker.sock\` | `wslc` 2.9.11 | Un répertoire créé sur l'hôte, sans erreur | Reproduit, non signalé [2026-09-13](#2026-09-13--compose-avec-wslc) |
| Supprimer des images et terminer une session rend de l'espace disque | `storage.vhdx` ne fait que grandir | `%LocalAppData%\wslc\sessions\<session>\storage.vhdx` | 814 → 1070 → 1550 Mo au fil d'un pull, d'un remove, d'un prune, d'un terminate et d'un nouveau pull | Reproduit ; `Optimize-VHD -Mode Full` en récupère une partie, 3995 → 2789 Mo en 10 s [2026-09-13](#2026-09-13--les-dernières-questions-ouvertes) |
| Deux moteurs qui publient le même port hôte entrent en conflit, ou au moins avertissent | `wslc` et Docker Desktop prennent tous deux le 8080 en silence, et celui qu'on atteint dépend du nom employé | réseau Windows, `wslc` 2.9.11 contre Docker Desktop 4.61 | `127.0.0.1` a toujours atteint `wslc` et `localhost` toujours Docker, dans les quatre variantes d'ordre de démarrage et d'adresse explicite | Reproduit, aucune erreur dans aucune variante [2026-09-13](#2026-09-13--les-dernières-questions-ouvertes) |
| `ContainerSettings.Privileged = true` donne un conteneur privilégié, comme `docker run --privileged` | Cela ne change rien : capacités, cgroups et `mount` reviennent identiques à `Privileged = false`, y compris dans une session administrateur. `wslcsdk.h` déclare `WSLC_CONTAINER_FLAG_PRIVILEGED = 0x4`, qui n'est pas appliqué | `wslc` 2.9.11 et son SDK | Masques `CapEff` et tables de montage des cgroups, avant et après ; k3s meurt toujours sur `failed to evacuate root cgroup: read-only file system` | Reproduit, non signalé. C'est pour cela qu'il n'y a pas de Kubernetes sur `wslc` [2026-09-13](#2026-09-13--aller-plus-loin--wslcimage-réseau-kubernetes-interface-graphique) |

## Expériences

Sept questions que le cours a mesurées au lieu de les supposer. Là où l'hypothèse était une ligne de documentation — les commentaires de `settings.yaml`, par exemple — la ligne la cite, puisque c'est elle qu'on éprouvait.

| Question | Hypothèse | Résultat | Verdict | Où |
|---|---|---|---|---|
| `cpuCount` et `memorySize` dans `settings.yaml` changent-ils ce que voit un conteneur ? | Les commentaires du fichier le disent : « Number of virtual CPUs allocated to the session… default: all » et « Memory limit… default: half of available memory » | Par défaut : 24 CPU et 31 946 Mo. Avec `cpuCount: 4, memorySize: 4GB` : `nproc` à 4, 3919 Mo de RAM et 4096 Mo de swap | Confirmée | [2026-09-13](#2026-09-13--limiter-le-cpu-et-la-mémoire-dune-session-wslc) |
| Modifier `settings.yaml` atteint-il une session déjà démarrée ? | Implicite : un réglage enregistré prend effet | La première mesure après la modification affichait encore 24 CPU ; il faut d'abord terminer la session | Réfutée | [2026-09-13](#2026-09-13--limiter-le-cpu-et-la-mémoire-dune-session-wslc) |
| Quand la VM d'une session inactive est-elle démontée ? | Au `idleTimeout` configuré, 30 s | En marche à 0 s, disparue à 35 s, en interrogeant toutes les 5 s : entre 30 et 35 s | Confirmée | [2026-09-13](#2026-09-13--limiter-le-cpu-et-la-mémoire-dune-session-wslc) |
| `memorySize` est-il une réservation ou un plafond, et la mémoire est-elle rendue à l'arrêt d'un conteneur ? | Pas écrite comme prédiction ; le tableau était l'objet même | VM au repos environ 921 Mo. En tenant 6 Go : 7069 Mo. Dix secondes après arrêt et suppression : 2776 Mo. Plus tard : 902 Mo | Un plafond, pas une réservation, et rendue progressivement plutôt que d'un coup | [2026-09-13](#2026-09-13--limiter-le-cpu-et-la-mémoire-dune-session-wslc) |
| `Optimize-VHD` récupère-t-il ce que `storage.vhdx` ne rend pas ? | Pas écrite comme prédiction | 3995 Mo à 2789 Mo en 10 s — encore au-dessus des 1,9 Go réellement occupés à l'intérieur | En partie : il en récupère, pas tout | [2026-09-13](#2026-09-13--les-dernières-questions-ouvertes) |
| `wslc` et Docker Desktop peuvent-ils publier tous deux le port 8080, et quel client atteint lequel ? | Pas écrite comme prédiction : la question était de savoir si ce serait seulement permis | Aucune erreur dans les quatre variantes. `127.0.0.1` a atteint `wslc` à chaque fois, `localhost` Docker à chaque fois | Confirmée, et bonne à savoir | [2026-09-13](#2026-09-13--les-dernières-questions-ouvertes) |
| Les options `--memory` et `--cpus` par conteneur contraignent-elles le conteneur ou la VM ? | Écrite à l'avance : elles devraient apparaître dans la vue du conteneur lui-même | `memory.max=536870912` et `cpu.max=150000 100000` correspondent exactement aux options, mais `nproc` dit toujours 8 et `free -m` toujours 15 996 Mo | Confirmée dans le cgroup, réfutée dans ce que voit le conteneur | [2026-09-16](#2026-09-16--lot-2--cinq-nouvelles-leçons) |
