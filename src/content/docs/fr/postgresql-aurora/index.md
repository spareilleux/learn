...
| 13 | [Migrer depuis SQL Server, et les coûts : une migration à la main, par programme et avec pgloader, AWS DMS, Babelfish, les prix d'Aurora](13-migration/) | le Data Migration Assistant, `SqlBulkCopy` |

Les leçons 1 à 11 se terminent chacune par une courte section sur ce qui change sur Aurora, d'après la documentation d'AWS ; la leçon 12 porte sur Aurora lui-même, avec un basculement reproduit en local ; la leçon 13 déplace une base SQL Server vers PostgreSQL, puis décrit AWS DMS, Babelfish et les prix d'Aurora. Rien de tout cela ne tourne sur AWS : le cours ne crée aucune ressource AWS, donc tout ce qui concerne Aurora est marqué *à vérifier*.
