...
Le compilateur TypeScript, .NET 10 et deux bibliothèques de validation. Plusieurs lignes décrivent un comportement documenté dans lequel une intuition vient se jeter, et la colonne État le dit. Les constantes du vérificateur ont été lues dans [`checker.go`](https://github.com/microsoft/typescript-go/blob/2bd066d87f5bafd315be9f40889d0a60b9e58e0b/internal/checker/checker.go) au commit épinglé avant d'être mesurées. Rien n'a été signalé en amont.

| Attendu | Ce qui se passe | Où | Mesure | État |
|---|---|---|---|---|
| Une limite de récursion au niveau des types est une propriété du type | C'est une propriété du fichier : les instanciations sont mises en cache, donc ce qui passe dépend de ce que le fichier a évalué avant | tsc 7.0.2, un `Reverse` non terminal | 48 éléments passent et 49 échouent quand il est seul dans le fichier ; avec un `Reverse` de 40 évalué d'abord dans le même fichier, 80 passent | Reproduit ; une limite mesurée dans un fichier n'est pas une limite pour un autre [2026-09-15](#2026-09-15--les-limites-du-vérificateur-mesurées) |
| Un type contextuel en littéral de gabarit élargit les littéraux vers `string` | Il les conserve : un type littéral de gabarit compte comme un contexte littéral | tsc 7.0.2, `satisfies Record<GovernanceHealthStatus, HexColor>` | Seul un type contextuel comme `string` les élargit | Par conception, et l'inverse de ce qu'attendait l'auteur [2026-09-15](#2026-09-15--les-prédictions-que-les-tests-de-types-ont-démenties) |
| Un paramètre de type `const` contraint par un tableau mutable retombe sur `string[]` | Il infère un tuple mutable | tsc, vérifié contre 5.0.4, 5.2.2 et 5.3.3 | Vrai de la 5.0 à la 5.2 ; changé en 5.3. La frontière a été située en bissectant trois versions épinglées | Reproduit ; un changement de comportement, correctement attribué [2026-09-15](#2026-09-15--les-prédictions-que-les-tests-de-types-ont-démenties) |
| `in out T` sur un type qui ne fait que lire `T` est refusé | Il est accepté, parce que `in out` rend un type invariant, ce qui est toujours sûr | tsc 7.0.2 | Seule une annotation qui contredit la structure échoue, en `TS2636` | Par conception [2026-09-15](#2026-09-15--les-prédictions-que-les-tests-de-types-ont-démenties) |
| Un `as` vers un type auquel il manque des propriétés est rejeté partout pareil | En direct, c'est `TS2352` ; en passant par une variable, les types sont comparables et cela compile | tsc 7.0.2 | La seconde forme est celle qu'on trouve dans du vrai code, et c'est celle que montre la leçon 2 | Par conception ; la moitié dangereuse est la silencieuse [2026-09-15](#2026-09-15--les-prédictions-que-les-tests-de-types-ont-démenties) |
| `dotnet run file.cs` peut sérialiser un type anonyme | Il échoue à l'exécution | programmes .NET 10 en fichier unique | `Reflection-based serialization has been disabled for this application`, parce que ces programmes se publient en AOT natif par défaut. `#:property PublishAot=false` le rétablit | Reproduit, non signalé [2026-09-15](#2026-09-15--le-côté-c-et-java) |
| `System.Text.Json` respecte par défaut les annotations de nullabilité et d'obligation | `RespectNullableAnnotations` et `RespectRequiredConstructorParameters`, tous deux ajoutés en .NET 9, sont désactivés par défaut | `System.Text.Json`, .NET 9+ | Une fois activés, le record C# devient un schéma | Par conception ; c'est le défaut qui surprend [2026-09-15](#2026-09-15--le-côté-c-et-java) |
| Importer un validateur élague ce qu'on n'utilise pas | L'import de baril, non : `z` est un objet qui référence toutes les fonctions | Zod 4.6.5 empaqueté par esbuild 0.28.2, minifié | 442,7 ko avec `import { z } from 'zod'` contre 87,4 ko avec `import * as z from 'zod'` | Reproduit ; la forme à espace de noms est aussi celle de la documentation [2026-09-15](#2026-09-15--les-tailles-de-bundle) |
| Des validateurs du même schéma ont des tailles comparables | Non | Valibot 1.5.0, `zod/mini`, Zod 4.6.5 | Valibot 4,8 ko minifié, environ 18 fois plus petit que Zod ; `zod/mini` environ 5 fois plus petit | Mesuré sous Windows, non signalé [2026-09-15](#2026-09-15--les-tailles-de-bundle) |
| `JsonHubProtocol` a besoin d'un serveur qui tourne pour montrer ce que SignalR envoie | Non | SignalR d'ASP.NET Core | `WriteMessage` sur un `InvocationMessage` donne le texte exact, ce qui a tranché ce que le hub de GA met sur le fil sans le démarrer | Une technique, pas un défaut — et c'est ce qui a permis de répondre hors ligne à une question de navigateur [2026-09-15](#2026-09-15--le-côté-c-et-java) |

## Expériences

Dans ce cours, une prédiction est une ligne de code : `type _1 = Expect<Equal<…>>`, que `tsc` vérifie avec le reste du projet, si bien qu'une prédiction fausse fait échouer `check.sh` avant que quoi que ce soit ne s'exécute. Quatre de ces six étaient fausses, et la section que le journal leur consacre s'appelle justement *les prédictions que les tests de types ont démenties*. La dernière ligne est la seule où le test a suivi le soupçon, et elle le dit.

| Question | Hypothèse | Résultat | Verdict | Où |
|---|---|---|---|---|
| Les constantes du vérificateur prédisent-elles où le code au niveau des types échoue vraiment ? | Lues d'abord dans `checker.go` : 100 instanciations imbriquées, 5 000 000 dans une expression, 1 000 pas pour un type conditionnel terminal, 100 000 membres pour une union en produit cartésien | Un constructeur de tuple terminal atteint 999 et échoue à 1 000 en `TS2589` ; une union de cinq positions de chiffres, 100 000 chaînes, échoue en `TS2590`. Les mesures concordent | Confirmée | [2026-09-15](#2026-09-15--les-limites-du-vérificateur-mesurées) |
| Où échoue un `Reverse` non terminal ? | Présentée comme difficile à prédire, et implicitement comme ayant un seul nombre pour réponse | 48 passent et 49 échoue quand il est seul dans le fichier ; 80 passent si un `Reverse` de 40 éléments est évalué d'abord dans le même fichier, les instanciations étant mises en cache | L'idée d'un nombre unique est réfutée | [2026-09-15](#2026-09-15--les-limites-du-vérificateur-mesurées) |
| `satisfies Record<GovernanceHealthStatus, HexColor>` élargit-il les couleurs vers `string` ? | Oui, écrite avant l'exécution du test de types | Il conserve les types littéraux : un type littéral de gabarit en type contextuel compte comme un contexte littéral | Réfutée, par une compilation en échec | [2026-09-15](#2026-09-15--les-prédictions-que-les-tests-de-types-ont-démenties) |
| Un paramètre de type `const` contraint par un tableau mutable retombe-t-il sur `string[]` ? | Oui, écrite avant l'exécution du test de types | Il infère un tuple mutable. L'attente valait de la 5.0 à la 5.2 et a changé en 5.3, vérifié contre 5.0.4, 5.2.2 et 5.3.3 | Réfutée, et la frontière de version située | [2026-09-15](#2026-09-15--les-prédictions-que-les-tests-de-types-ont-démenties) |
| `in out T` est-il refusé sur un type qui ne fait que lire `T` ? | Oui, écrite avant l'exécution du test de types | Accepté : `in out` rend un type invariant, ce qui est toujours sûr. Seule une annotation contradictoire échoue, en `TS2636` | Réfutée | [2026-09-15](#2026-09-15--les-prédictions-que-les-tests-de-types-ont-démenties) |
| Le `as` de `stopFrom` cache-t-il quelque chose ? | Soupçonné, mais le test de types a été ajouté après — cette ligne est la seule entrée rétrospective du tableau | `EventOf<LiveState>` vaut `never` : l'assertion masquait que l'état `stopped` n'a pas d'événement `stop`. La fonction prend désormais `Exclude<LiveState, 'stopped'>`, sans assertion | Réfutée, après coup | [2026-09-15](#2026-09-15--les-prédictions-que-les-tests-de-types-ont-démenties) |
