...
|---|---|---|---|---|
| Un `COPY` dans une table de relations depuis une sous-requête relie les nœuds trouvés par le `MATCH` | Il relie les mauvais nœuds : les clés sont appariées par position plutôt que par le `MATCH` | `COPY … FROM (LOAD FROM … MATCH …)` | Reproduction 1 [2026-09-14](#2026-09-14--bugs-trouvés-dans-la-0204) | Reproduit, aucun ticket amont, non signalé |
| `count(DISTINCT …)` à côté de `sum(…)` laisse la somme tranquille | La `sum` revient à `NULL` dès qu'un `count(DISTINCT …)` la précède | Agrégation | Reproduction 2 [2026-09-14](#2026-09-14--bugs-trouvés-dans-la-0204) | Reproduit, aucun ticket amont, non signalé |
| `ACYCLIC` écarte tout chemin qui répète un nœud | Il en garde certains | Motifs de chemins récursifs | Reproduction 3 [2026-09-14](#2026-09-14--bugs-trouvés-dans-la-0204) | Reproduit, aucun ticket amont, non signalé |
| Un `COPY` qui échoue laisse la table de nœuds dans son état | Il la corrompt | `COPY` dans une table de nœuds | Reproduction 4 [2026-09-14](#2026-09-14--bugs-trouvés-dans-la-0204) | Reproduit, aucun ticket amont, non signalé |
| Grouper par une sous-requête `COUNT` conserve le groupe dont le compte vaut 0 | Ce groupe disparaît | `GROUP BY` sur une sous-requête | Reproduction 5 [2026-09-14](#2026-09-14--bugs-trouvés-dans-la-0204) | Reproduit, aucun ticket amont, non signalé |
| `timestamp()` et `CAST(… AS TIMESTAMP)` respectent le décalage écrit dans le littéral | Les deux l'ignorent | Conversion temporelle | Reproduction 6 [2026-09-14](#2026-09-14--bugs-trouvés-dans-la-0204) | Reproduit, aucun ticket amont, non signalé |
...
## 2026-09-14 — Leçon 8 : transactions, fichiers et processus

- Une erreur dans une transaction manuelle annule toute la transaction et y met fin, sans message qui le dise : les erreurs d'exécution (une clé en double), les erreurs de liaison, les erreurs de catalogue, un `BEGIN` dans une transaction et `CHECKPOINT` dans une transaction le font toutes. Les instructions qui suivent s'exécutent en auto-commit ; `ROLLBACK` et `COMMIT` échouent ensuite avec `No active transaction`. La seule exception trouvée : une écriture dans `BEGIN TRANSACTION READ ONLY` échoue, et la transaction reste ouverte. La [page sur les transactions](https://docs.ladybugdb.com/cypher/transaction/) promet que rien d'une transaction en échec ne persiste, et ne dit pas que les instructions après l'erreur s'exécutent seules.
- Une seconde transaction d'écriture est refusée aussitôt, `Cannot start a new write transaction in the system. Only one write transaction at a time is allowed in the system.`, même quand elle touche d'autres nœuds ; il n'y a ni attente ni délai d'expiration. Une instruction en auto-commit refusée laisse la connexion utilisable.
- Java, **un `BEGIN TRANSACTION` refusé casse la connexion** : la requête suivante lève `RuntimeException: Unknown Error` sous Windows, et fait planter la JVM avec `SIGSEGV` sous Linux (`CatalogSet::traverseVersionChainsForTransactionNoLock`) et macOS (`CatalogSet::containsEntry`), code de sortie 134. Un `BEGIN` qui réussit répare la connexion sous Windows ; un `BEGIN` retenté ne plante sur aucun des trois runners (exercice 2 : 153, 155 et 24 refus). Reproduction minimale, une `Database` en mémoire et deux connexions : `first` exécute `BEGIN TRANSACTION`, `second` exécute `BEGIN TRANSACTION` (refusé), `first` exécute `COMMIT`, `second` exécute n'importe quel `MATCH`. Le paquet C# et les autres bindings n'ont pas été testés.
- Les fichiers : le `.wal` apparaît au premier commit et disparaît au point de contrôle quand la base de données se ferme ; pas de `.wal` pour une transaction non validée. `kill -9` après un commit laisse le `.wal`, rejoué à l'ouverture suivante ; `kill -9` dans une transaction perd la transaction. Fermer une `Database` Java avec une transaction ouverte l'annule.
- Un second processus en lecture-écriture échoue avec `Could not set lock on file`, suivi de `(Error: 33)` sous Windows et de `(Error: Resource temporarily unavailable)` sous Linux. Le code POSIX ferme le descripteur de fichier avant `F_GETLK`, donc son message `Lock is held by PID` ne peut pas apparaître ([`local_file_system.cpp`, lignes 147-172](https://github.com/LadybugDB/ladybug/blob/v0.20.4/src/common/file_system/local_file_system.cpp#L147-L172)).
- **Une base de données en lecture seule ne prend aucun verrou** ([`storage_manager.cpp`, lignes 87-91](https://github.com/LadybugDB/ladybug/blob/v0.20.4/src/storage/storage_manager.cpp#L87-L91)). Un processus en lecture seule ouvre un fichier que tient un processus d'écriture sous Linux et macOS, et voit les modifications validées de ce dernier depuis le `.wal` ; pas sous Windows, où le `LockFileEx` du processus d'écriture fait échouer la lecture avec `Error 33`. Un processus d'écriture ouvre, modifie et fait un point de contrôle d'un fichier que tient un processus en lecture seule, sur les trois OS, et le lecteur continue de répondre depuis l'ancien état. La [page sur la concurrence](https://docs.ladybugdb.com/concurrency/) dit que cette combinaison n'est pas permise.
- Dans un même processus Java, une seconde `Database` en lecture-écriture, ou en lecture seule, sur un fichier que tient une `Database` en lecture-écriture : refusée sous Windows, ouverte sous Linux et macOS (les verrous `fcntl` appartiennent au processus).
- `new Database(path)` lève `java.lang.Exception`, une exception vérifiée qu'il ne déclare pas ([`lbug_java.cpp`, lignes 618-636](https://github.com/LadybugDB/ladybug-java/blob/f2fb39f/src/jni/lbug_java.cpp#L618-L636)).
- `EXPORT DATABASE` en CSV écrit les macros dans `schema.cypher` ; la [page sur la migration](https://docs.ladybugdb.com/migrate/) mentionne un fichier `macro.cypher` qui n'est pas créé. Les en-têtes CSV sont préfixés par `a.`. L'ordre des options dans `copy.cypher` diffère entre Windows et Linux ou macOS. `IMPORT DATABASE` dans une base de données non vide échoue avec `Project already exists in catalog.`
- La 0.20.4 ouverte en lecture seule laisse un fichier 0.19.1 tel quel ; ouverte en lecture-écriture, elle le met à niveau de la version de stockage 43 à 47 sans message, et la CLI 0.19.1 affiche ensuite `Database file version: 47, Current build storage version: 43`.
- CI : la première exécution a échoué sur les trois jobs `cypher`, parce que `files.sh`, exécuté sous `set -e`, renvoyait le code de sortie de la dernière CLI 0.19.1 ; `check.sh` l'ignore désormais et compare la sortie. Les jobs `java` sous Linux et macOS ont planté au `BEGIN` refusé, désormais dans un mode à part. La deuxième exécution a échoué sous macOS seulement : `ls` triait `Project.csv` après `copy.cypher` ; le script trie avec `LC_ALL=C`.
- Quand sa sortie est redirigée vers un fichier, la CLI n'écrit rien avant de se terminer : `files.sh` attend le fichier `.wal`, ou un temps fixe, plutôt qu'une ligne de sortie.
...
- Si un processus en lecture seule qui reste ouvert pendant qu'un processus d'écriture fait un point de contrôle renvoie des données fausses, et pas seulement anciennes, quand il lit des pages qu'il n'avait pas en cache (leçon 8).
- Sous Linux et macOS, si fermer la seconde `Database` d'un processus libère le verrou `fcntl` de la première, comme fermer n'importe quel descripteur d'un fichier libère les verrous du processus sur ce fichier, et permet à un autre processus d'ouvrir le fichier en lecture-écriture.
- Si un `BEGIN` refusé casse aussi une connexion du paquet C# et des autres bindings.
