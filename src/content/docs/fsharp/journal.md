...
- `ChordParser.parse` now requires the whole symbol ([#682](https://github.com/GuitarAlchemist/ga/pull/682)) and reads `7sus4`, `Maj7` and `omit` ([#689](https://github.com/GuitarAlchemist/ga/pull/689)), both merged 2026-09-23. Re-run on GA [`27a1257`](https://github.com/GuitarAlchemist/ga/commit/27a1257f7fe5478c4b1bcb6b86ebe101707ac935) by music-theory-ga's [`recheck/probe.cs`](https://github.com/spareilleux/learn/blob/main/code/music-theory-ga/recheck/probe.cs): `C7sus4` → `Ok` with `Components = [Extension "7"; Extension "sus4"]`, and `Am(maj7)` → `Error in Ln: 1 Col: 3`. Lesson 4 stays pinned to `32f143c` and still shows the prefix being accepted.
- `GetNormalForm` is unchanged: its file is identical at `32f143c` and `27a1257`.
- [#734](https://github.com/GuitarAlchemist/ga/pull/734), merged 2026-09-24, fixes `GetNormalForm`: ties of span are broken as Rahn does, by the intervals from the first element to the second-to-last, then the third-to-last, and so on. `{0, 4, 7, 8}` now gives `[0; 3; 4; 8]`, the same as its transposition by 8. Its test runs every one of the 4,096 sets under T1 to T11; on the old code, 5,920 of those 45,045 transpositions changed the answer. The doc comment no longer calls the result a Forte prime form: it is the normal order transposed to zero, without the inversion. A last test pins `{0, 1, 5, 6, 8}`, a set where Rahn and Forte disagree.
