...
## 2026-09-25 — Upstream fixes

- [#736](https://github.com/GuitarAlchemist/ga/pull/736), merged 2026-09-25, fixes the five GA rows. Each change has a test that fails on the old code: 51 of the 57 round-trip tests, 22 of the 35 formatter tests, the trace test, and 4 of the client's drawing tests.
- **The parser.** `str`, `ch` and `pint` no longer consume the whitespace after them. The same bug made `key=G` followed by a newline read the `n` of the next line's `notes` as a natural sign. The parser and the generator now follow VexTab where the tutorial and `vextab.jison` are clear: techniques between frets (`5h7/3`), `@` for flats in notes, `key=Am`, durations mid-line, fret runs, taps, annotations as note items, VexTab's order of duration flags. Run through this course's harness on 25 texts, GA and VexTab 4.0.5 now agree on every one; for the 20 both accept, the text GA writes back draws an SVG byte-identical to the original's. Not done, and listed in the PR: `stave` and `voice` lines, ties, slurs, tuplets, the durations `:1 :2 :4 :64`, and `X` as a note name.
- **The chatbot.** Its blocks are VexTab now: open C is `tabstave` / `notes :w (3/5.2/4.0/3.1/2.0/1)`, and the prompt asks for that form. `MemoizedVexTab` parses a block and draws a tab stave with VexFlow 4.2.5; a block it cannot draw whole stays as text, and `data-renderer` on the block says which the reader sees. The constant `notation.renderer` is gone: the server draws nothing, so the traces now report `notation.vextab.block_count` and `valid_count`.
- **What the course had left to verify.** No GA workflow runs ga-client's Playwright specs: `ci.yml` only lints and builds the client. And `vextab-rendering.spec.ts` could not pass as written, because its `beforeEach` clicks an "AI Chat Assistant" tab that no longer exists. Read in GA's workflows and spec, not run.
- The 45 golden traces pin the old `notation.renderer` field; they come from live runs and need re-recording on a machine with the model. While reading the chat client, the same session found `/ai-copilot` crashing on its first render; that was fixed by [#737](https://github.com/GuitarAlchemist/ga/pull/737).
