...
OpenJDK 25+36, Maven 3.9.16, Gradle 9.7.1, compared throughout with the .NET 10 SDK. Several rows are defaults that differ from .NET's, where the Java behaviour is documented and the surprise is only in bringing the C# habit across; the Status column says which. The first two rows come from building GuitarAlchemist/ga's JetBrains plugin, which is where this journal's practical cases start. Nothing here has been reported upstream.

There is no Experiments table: no entry states a hypothesis before the measurement that tests it. The one sentence shaped like a hypothesis — that the first racy run lost more than 90% of its updates because the JIT had not yet compiled the loop — explains a number already recorded, and the journal marks it *to verify*.

| Expected | What happens | Where | Measure | Status |
|---|---|---|---|---|
| Gradle names the problem when the JDK is too new for it | Gradle 8.5 on JDK 25 fails with a message that is only the Java version number | `jetbrains-plugin/` of GuitarAlchemist/ga at `5560b883`, Gradle 8.5, JDK 25 | `* What went wrong:` followed by `25`; the stack trace shows `java.lang.IllegalArgumentException: 25` thrown while Gradle compiles the Kotlin build script | Reproduced; upgrading Gradle moves the failure to the next row [practical cases](#practical-cases-in-my-repositories) |
| A Gradle version that supports JDK 25 is enough to build the plugin | Gradle 9.7.1 gets further and dies inside the legacy `org.jetbrains.intellij` 1.16.1 plugin | the same build, Gradle 9.7.1 | `class org.jetbrains.intellij.MemoizedProvider overrides final method org.gradle.api.internal.provider.AbstractMinimalProvider.toString()` | Reproduced; migrating to the IntelliJ Platform Gradle Plugin 2.x is the next step, not done [practical cases](#practical-cases-in-my-repositories) |
| Maven warns when it resolves a dependency older than one a module asked for | It puts Commons Lang 3.14.0 on the class path against a request for 3.20.0, silently, and the program fails at run time | Maven 3.9.16, `l10/check.sh` | `NoClassDefFoundError` at run time; `-Dverbose` is needed even to see that another version was requested. NuGet's `NU1605` makes the same situation an error by default | By design [2026-09-13](#2026-09-13--lesson-10) |
| Nearest-wins gives a library and its agent artifact matching versions | Mockito 5.23.0 asks for Byte Buddy 1.17.7, AssertJ 3.27.7 for 1.18.3, and Maven pairs `byte-buddy` 1.18.3 with `byte-buddy-agent` 1.17.7 | `l11`, Maven 3.9.16 | It works, because AssertJ's path is shorter and brings the newer version — by luck, not by rule | Reproduced, not reported [2026-09-13](#2026-09-13--lesson-11) |
| `CompletableFuture.cancel(true)` interrupts the running task, as `mayInterruptIfRunning` says | The future reports cancelled while its task runs to completion | `CompletableFuture`, OpenJDK 25+36 | The Javadoc says the argument "has no effect in this implementation". A cancelled five-second task still took five seconds, because the executor's `close()` waited for it | By design, and documented; lesson 9 now shows it with a one-second task [2026-09-13](#2026-09-13--lesson-9) |
| BlockHound installs with `BlockHound.install()`, as its README shows | On JDK 25 self-attach fails, the documented workaround fails differently, and only `-javaagent` works | BlockHound 1.0.17, JDK 25, `l11` | `Could not self-attach to current VM using external process`; with `-Djdk.attach.allowAttachSelf=true`, `Agent JAR loaded but agent failed to initialize` | Reproduced, not reported; lesson 11 uses `-javaagent` [2026-09-13](#2026-09-13--lesson-11) |
| A current instrumentation library needs no deprecated JVM flag and names the method you called | BlockHound needs a flag deprecated since JDK 13, and reports `Thread.sleep` under a private JDK name | BlockHound 1.0.17, JDK 25, `l11/check.sh` | `-XX:+AllowRedefinitionToAddDeleteMethods`; `Thread.sleep` reported as `java.lang.Thread.sleepNanos0` | Reproduced, not reported [2026-09-13](#2026-09-13--lesson-11) |
| A failing annotation processor reports its error | Maven prints a generic message and hides the cause unless run with `-e` | NullAway 0.14.1 on Error Prone 2.50.0, JDK 25, `l11` | `An unknown compilation problem occurred`; the real error, an `IllegalAccessError` on javac's internal API, appears only with `mvn -e` | Reproduced; `.mvn/jvm.config` with `--add-exports` lines fixes it [2026-09-13](#2026-09-13--lesson-11) |
| `javac -XDrawDiagnostics` and the `javax.tools` API give the same diagnostic key | They diverge on the same rejected snippet | OpenJDK 25+36, adding to a `List<? extends Number>` | The command line gives `compiler.err.cant.apply.symbols`, the API `compiler.err.prob.found.req` | Reproduced; the lessons quote what the API sees, because the tests use the API [2026-09-13](#2026-09-13--lessons-1-to-4) |
| `YYYY` in a date pattern is the calendar year, as in .NET | It is the week-based year, which differs at year end and depends on the locale | Java date formatting, lesson 12 | December 31, 2026 printed `2027` with `Locale.US` and `2026` with `Locale.FRANCE` | By design [2026-09-13](#2026-09-13--lesson-12) |
| Java's `HttpClient` follows redirects and times out, as .NET's does | It does neither unless configured | `java.net.http.HttpClient`, JDK 25 | Ported .NET code gets a 302 with an empty body, or a call that can wait forever | By design [2026-09-13](#2026-09-13--lesson-12) |
| Surefire shows `@DisplayName` and credits tests to the class that declares them | It ignores `@DisplayName` without a reporter option, and credited a whole class's tests to its nested class | Maven Surefire with JUnit 6, `l11` | All nine tests of `PriceCalculatorTest` credited to its nested class in the console output | Reproduced, not reported [2026-09-13](#2026-09-13--lesson-11) |
| An immutable `Map.of(…)` iterates in a stable order | The order changes from one JVM start to the next | `Map.of`, OpenJDK 25+36 | Four runs of the same one-liner gave two orders | By design: `Map.of` randomises its iteration order per JVM. The example's output no longer depends on it [2026-09-13](#2026-09-13--lessons-5-to-8) |
