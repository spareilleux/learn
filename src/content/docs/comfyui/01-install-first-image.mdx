...
Get your own machine to the same place, then make it say so. Start the server, queue the lesson's graph with `curl`, and read the three numbers this lesson read: how long the first image took, how long the second took with the models already loaded, and how much memory `nvidia-smi` showed while it sampled. Then change the prompt to something you care about — an object on your desk — and keep the first image you get, however wrong it is. Lesson 2 explains why it is wrong, and the comparison only means something if you have your own before.
