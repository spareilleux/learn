...
| A node that prepares an inpainting latent leaves the pixels alone | `VAEEncodeForInpaint` pushes every masked pixel to mid grey before encoding, which the inpainting tutorial does not mention | [`nodes.py:443-447`](https://github.com/Comfy-Org/ComfyUI/blob/ee71d5c4993f29086b27fde1629a945ae48425bf/nodes.py#L443-L447) | At `denoise` 0.5 the masked area came back a flat grey ellipse; `denoise` 1 with a 24-pixel soft edge, or the inpainting UNet, gives a usable result | Reproduced. The node is built for `denoise` 1 |
| A LoRA's card describes the file it ships | Pixel Art XL is published under CreativeML Open RAIL-M, not the RAIL++-M of the SDXL it extends, and its card says no trigger word is needed while the file's header sets `instance_prompt: pixel art` | [nerijs/pixel-art-xl at `8bf4a4d9`](https://huggingface.co/nerijs/pixel-art-xl/tree/8bf4a4d9) | Licence read on the card at download time, `instance_prompt` read in the safetensors header | Reproduced. Read the header, not the card |
| Something inspects a custom-node pack before it runs | Nothing does. Each pack is imported with `exec_module` before its `NODE_CLASS_MAPPINGS` is read, a `prestartup_script.py` runs earlier still, and a pack's `WEB_DIRECTORY` JavaScript runs in the browser | [`nodes.py:2266`](https://github.com/Comfy-Org/ComfyUI/blob/ee71d5c4993f29086b27fde1629a945ae48425bf/nodes.py#L2266) | `comfyui_manager` 4.2.2 gates installs by security level, listener and two flags and matches known bad package names at startup; no step reads a pack's code. The 42 files of its PyPI wheel were identical to the `4.2.2` tag, commit `bd4ede22` | Reproduced. Installing a pack is running its code |
| Loading a checkpoint does not execute code | A pickle whose payload calls `print` ran under `pickle.loads` and under `torch.load(weights_only=False)`; `torch.load(weights_only=True)` refused it on PyTorch 2.13 | `comfyui_controlnet_aux`, pinned 16 September 2026 — a pack, not ComfyUI core | 72 `torch.load` calls without `weights_only`, which defaults to safe only on PyTorch 2.6 and later | Reproduced |
| Installing a pack is the only step that reaches the network | ComfyUI-Impact-Pack pip-installs `onnxruntime` the first time an ONNX detector runs, and its `install.py` downloads a SAM `.pth`; rgthree-comfy sends a model file's SHA-256 to Civitai when the interface asks for that model's information | `ComfyUI-Impact-Pack/install.py` and `rgthree-comfy`, both pinned 16 September 2026 | Audit of six packs: ComfyUI-GGUF, comfyui_controlnet_aux, ComfyUI-Impact-Pack, ComfyUI-VideoHelperSuite, rgthree-comfy, ComfyUI_essentials | Reproduced. None of it suggests bad intent; it is what a pack may do without saying so |
