...
PostgreSQL 18.6 in a container, Npgsql 10.0.3, pgjdbc 42.7.13, pgvector 0.8.6, pgloader from source, and SQL Server 2025 for the migration. Most rows are a driver, a tool or a default doing something the documentation or the other driver does not; two are GuitarAlchemist/ga and AWS's own documentation. Nothing here has been reported upstream.

There is no Experiments table. The journal records no hypothesis before any of its measurements: the five HNSW rebuilds, the statistics-target threshold and the driver timings are measurements whose explanation was written after the number, and turning them into experiments now would mean inventing the prior backwards.

| Expected | What happens | Where | Measure | Status |
|---|---|---|---|---|
| `CREATE FUNCTION … BEGIN ATOMIC …; END` is one command, since its semicolons are inside the body | Npgsql splits the command text at the semicolon inside the body, as it does between statements, and the server rejects the truncated command | Npgsql 10.0.3, `NpgsqlCommand` without parameters | `42601: syntax error at end of input`. It works in an `NpgsqlBatch`, with `Npgsql.EnableSqlRewriting` off, or with positional parameters. pgjdbc 42.7.13 recognises `BEGIN ATOMIC` and does not split | Reproduced, not reported [2026-09-16](#2026-09-16--functions-and-pgvector) |
| pgloader migrates a SQL Server `money` column without losing digits, or warns | It keeps two decimals, silently: it reads `money` through `convert(varchar(40), [col], 0)`, and style 0 truncates | pgloader built from `231ab86`, [`mssql-schema.lisp`, lines 212-217](https://github.com/dimitri/pgloader/blob/231ab86778ca5ffd7de40878714760c8b4860cdf/src/sources/mssql/mssql-schema.lisp#L212-L217) | `0.0125` became `0.01`, with no error or warning. The same run dropped the `CHECK` constraint and turned `INCLUDE (Conclusion)` into a key column | Reproduced, not reported [2026-09-16](#2026-09-16--migrating-from-sql-server) |
| `Target Session Attributes=standby` is accepted with a single host, as pgjdbc accepts `targetServerType=secondary` | Npgsql refuses the connection string outright; pgjdbc accepts it and fails later, at connection time | Npgsql 10.0.3, pgjdbc 42.7.13 | `NotSupportedException: Target Session Attributes other then Any is only supported with multiple hosts` | By design in both, differently [2026-09-15](#2026-09-15--the-drivers) |
| PowerShell passes `-Duser.timezone=Asia/Tokyo` to `java` as one argument | PowerShell 7.6 still splits it at the dot | PowerShell 7.6 | `Could not find or load main class .timezone=Asia.Tokyo` | Reproduced; quoted, it works [2026-09-15](#2026-09-15--the-drivers) |
| An unspecified-kind `DateTime` gives the same rows whatever the session's time zone | It does not throw, as expected, but it is sent as `timestamp` and converted in the session's time zone | Npgsql 10.0.3 | 82 runs or 59, depending on `Timezone`. A `DateTimeOffset` with a −4 hour offset throws `ArgumentException` instead | By design [2026-09-15](#2026-09-15--the-drivers) |
| `SET statement_timeout = '2s'` ends an `INSERT` waiting for a stopped synchronous standby | The wait is not interrupted; the statement hangs until it is cancelled from outside | PostgreSQL 18.6, synchronous replication between the course container's clusters | Ten minutes, until `pg_cancel_backend`; the warning says the transaction "has already committed locally" | By design: the commit is already local and only the wait remains. The script now cancels the wait from another session [2026-09-16](#2026-09-16--several-servers-in-one-container) |
| `pg_rewind` rewinds the demoted primary against the new one | The old primary had already recycled the WAL segment `pg_rewind` needs | `pg_rewind` 18, the course container's `node1` | `could not open file "node1/pg_wal/000000010000000000000002"` | Reproduced; `wal_keep_size = 128MB` keeps the segment [2026-09-16](#2026-09-16--several-servers-in-one-container) |
| `pg_upgrade --check` from a default 17 cluster to a default 18 cluster passes | `initdb` 18 enables data checksums by default, so the two defaults no longer match | `pg_upgrade` 18, `initdb` 18 | Stopped at "old cluster does not use data checksums but the new one does" | By design, a changed default; `--no-data-checksums` on the new cluster [2026-09-16](#2026-09-16--backups-and-pg_upgrade) |
| A `jsonb_path_ops` GIN index is smaller than a `jsonb_ops` one, as the documentation says is usual | On these documents it is slightly larger | PostgreSQL 18.6, the copied CI documents | 1,752 kB for `jsonb_path_ops` against 1,688 kB for `jsonb_ops` | Reproduced; "usual" is not "always" [2026-09-16](#2026-09-16--json-and-search) |
