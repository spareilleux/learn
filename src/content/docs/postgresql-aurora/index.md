...
[PostgreSQL](https://www.postgresql.org/docs/18/) **18.6**, the current minor release of the newest major version on 2026-09-15 according to the [versioning policy page](https://www.postgresql.org/support/versioning/) (PostgreSQL 18 is supported until November 2030), in the official Docker image [`postgres:18.6-trixie`](https://hub.docker.com/_/postgres), digest `sha256:4ef4dbc939d61acea57712655ddb4b4ab27419c913f94cca0cd57cb3ea3c2280`. Lesson 8's [pgvector](https://github.com/pgvector/pgvector) scripts run on [`pgvector/pgvector:0.8.6-pg18-trixie`](https://hub.docker.com/r/pgvector/pgvector), the same PostgreSQL 18.6 with pgvector **0.8.6**, digest `sha256:78bf48b801e792f99e3ac62b5036fd3876e9be48afda16c1e331af1c75ceb2ff`. Lesson 11's upgrade installs PostgreSQL 17's binaries in the container from the PostgreSQL project's apt repository, whose latest 17.x minor release changes over time. The clients are [Npgsql](https://www.npgsql.org/) **10.0.3** and its [EF Core provider](https://www.npgsql.org/efcore/) **10.0.3** on .NET 10, and the [PostgreSQL JDBC driver](https://jdbc.postgresql.org/) **42.7.13** with [HikariCP](https://github.com/brettwooldridge/HikariCP) **7.1.0** on Java 25. Lesson 13 migrates from [SQL Server 2025](https://learn.microsoft.com/sql/sql-server/what-s-new-in-sql-server-2025) CU9, in Microsoft's image `mcr.microsoft.com/mssql/server:2025-CU9-ubuntu-24.04`, digest `sha256:2b5b581621126574f3d1f75e78d3eebe8d05aedb59ad0cfdf9aa42cb0634d726`, with [Microsoft.Data.SqlClient](https://learn.microsoft.com/sql/connect/ado-net/introduction-microsoft-data-sqlclient-namespace) **7.0.3** and [pgloader](https://pgloader.io/) `3.6.10~devel`, image `ghcr.io/dimitri/pgloader` digest `sha256:a1d4a78e78a64e46cd3fc7dfc57d24eb91ffb1a5520f2b1f55631815e3658d6e`. On AWS, the newest release listed in the [Aurora PostgreSQL release notes](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraPostgreSQLReleaseNotes/AuroraPostgreSQL.Updates.html) on the same day is **Aurora PostgreSQL 18.4.1** (August 21, 2026), compatible with PostgreSQL 18.4.

Every example is in [`code/postgresql-aurora`](https://github.com/spareilleux/learn/tree/main/code/postgresql-aurora): [`check.sh`](https://github.com/spareilleux/learn/blob/main/code/postgresql-aurora/check.sh) runs each SQL script with `psql`, each C# and Java program against a fresh database, the replication and backup scripts of lessons 10 and 11 on small clusters started inside the container, and lesson 13's migration from a SQL Server container, and compares their output with the files in [`expected`](https://github.com/spareilleux/learn/tree/main/code/postgresql-aurora/expected). CI builds the programs on Windows, Linux and macOS, and runs everything against PostgreSQL, and SQL Server for lesson 13, on Linux only, because GitHub's Windows and macOS runners have no Docker.
:::
...
| 9 | [Partitioning and large tables](09-partitioning/) | partition functions and schemes, `SWITCH` |
| 10 | [Replication and high availability: WAL, physical and logical replication](10-replication/) | Always On availability groups, transactional replication |
| 11 | [Backup, restore and upgrades: `pg_dump`, point-in-time recovery, `pg_upgrade`](11-backup/) | `BACKUP`, `RESTORE … STOPAT`, in-place upgrades |
| 12 | [Amazon Aurora PostgreSQL: storage, replicas and failover, endpoints, Aurora Serverless, Global Database, RDS Proxy, IAM authentication](12-aurora/) | Azure SQL Database, Hyperscale |
| 13 | [Migrating from SQL Server, and costs: a migration by hand, by program and with pgloader, AWS DMS, Babelfish, Aurora prices](13-migration/) | the Data Migration Assistant, `SqlBulkCopy` |

Lessons 1 to 11 each end with a short section on what changes on Aurora, from AWS's documentation; lesson 12 is about Aurora itself, with a failover reproduced locally; lesson 13 moves a SQL Server database to PostgreSQL, then describes AWS DMS, Babelfish and Aurora's prices. None of it runs on AWS: the course creates no AWS resource, so everything about Aurora is marked *to verify*.
