...
| 13 | Migration and costs: from SQL Server to PostgreSQL and Aurora with AWS DMS and Babelfish *(coming next)* | the Data Migration Assistant |

Lessons 1 to 11 each end with a short section on what changes on Aurora, from AWS's documentation; lesson 12 is about Aurora itself, with a failover reproduced locally. None of it runs on AWS: the course creates no AWS resource, so everything about Aurora is marked *to verify*.
