...
V 0.5.2 en la etiqueta `7647ce1`. Varias filas enfrentan la documentación con la versión instalada, y una de ellas es un compilador que envía tu fuente a un servidor mientras no conozcas la variable de entorno. Nada se ha reportado: el diario lo dice sin rodeos, y precisa que el tracker ni siquiera se buscó.

No hay tabla de experimentos: ninguna entrada de este diario plantea una hipótesis antes de la medición que la pone a prueba. Las dos comparaciones construidas — los tres regímenes de memoria y veinte ejecuciones repetidas de una carrera de datos — son mediciones sin supuesto registrado, y se quedan en sus entradas fechadas.

| Esperado | Lo que pasa | Dónde | Medición | Estado |
|---|---|---|---|---|
| El compilador no envía nada a ninguna parte | V envía por defecto los errores del compilador de C a un servidor. El informe lleva el C generado que falla y las líneas de fuente V, con las rutas de la máquina del autor | `vlib/v/builder/c_error_report.v` 485-506, V 0.5.2 | `Sent C compiler bug report to https://bugs.vlang.io/bug-report.`, con un identificador de informe y un comando `curl -X DELETE` para borrarlo. Envía salvo que se ponga `V_C_ERROR_BUG_REPORT_DISABLED`, y se abstiene en la CI de GitHub | Reproducido. La única mención encontrada está en `v help build-c`, no en el `docs.md` de la 0.5.2. `check.sh` y las ejecuciones locales ponen ya la variable [2026-09-14](#2026-09-14--sorpresas-al-escribir-las-lecciones-5-8) |
| `-autofree` libera casi todo y el recolector se encarga del resto, como dice la documentación | `-autofree` apaga el recolector por completo: lo que autofree se deja, se filtra | `vlib/v/pref/pref.v` 807-811 | `gc_is_enabled()` devuelve `false` | Reproducido; la documentación y el código se contradicen [2026-09-14](#2026-09-14--sorpresas-al-escribir-las-lecciones-1-4) |
| `-prod` cambia la optimización, no la gestión de memoria | `-prod` con MSVC en Windows y sin opción `-gc` pasa a `no_gc`, una regla que el autor no encontró documentada en ninguna parte | `vlib/v/pref/default.v` 369-381 | Tras 50 vueltas: 20 MB con `v run`, 542 MB con `v -prod run`, 30 MB con `v -prod -gc boehm run` | Reproducido; el ejecutor de Windows de la CI usa `gcc` para `-prod`, así que nunca lo muestra [2026-09-14](#2026-09-14--sorpresas-al-escribir-las-lecciones-1-4) |
| Una función anónima dentro de un `lock` compila y se ejecuta | V la acepta y luego TCC falla: el C generado de la función anónima contiene el desbloqueo del bloque que la envuelve | generación de código, V 0.5.2 | `'usage' undeclared`, de `sync__RwMutex_runlock(&usage->mtx);return _t1;`. Ocurre con `lock` y con `rlock`, capturando o no | Reproducido; fijado en CI como `compiler_bugs/b07_return_in_lock.v` para que una corrección se note. El tracker de V nunca se buscó, nada se reportó [2026-09-14](#2026-09-14--sorpresas-al-escribir-las-lecciones-5-8) |
| Empujar a un canal cerrado entra en pánico, como dice la documentación | `ch <- x` sobre un canal cerrado no hace nada: el `pushval` generado llama a `try_push_priv` e ignora el `.closed` devuelto | `vlib/v/gen/c/cgen.v` 2492-2501 | `popval` igualmente devuelve un valor cero en un canal cerrado y vacío. Con `or`, ambos informan `channel closed` | Reproducido, no reportado [2026-09-14](#2026-09-14--sorpresas-al-escribir-las-lecciones-5-8) |
| Acceder a un campo que solo tiene una variante de un tipo suma es un error de compilación | Compila como conversión implícita y entra en pánico en ejecución | `vlib/v/checker/checker.v` 3180-3201 | `as cast: cannot cast main.Floating to main.Release` | Reproducido; comprobado en CI por `panics/p06_variant_field.v` [2026-09-14](#2026-09-14--sorpresas-al-escribir-las-lecciones-5-8) |
| Un literal flotante dividido por un entero pasa a ser `f64` | `7.0 / 2` conserva el tipo `float literal` | `vlib/v/ast/types.v` 1278 | `typeof(y).name` imprime `float literal` donde `z := 3.5` da `f64`; `println(y)` imprime igualmente `3.5` | Reproducido [2026-09-14](#2026-09-14--sorpresas-al-escribir-las-lecciones-1-4) |
| Convertir una cadena a número avisa del fallo | Nunca falla | `string.int()`, V 0.5.2 | `'12abc'.int()` vale 12, `'abc'.int()` vale 0, `'99999999999'.int()` vale 2147483647. `strconv.atoi` avisa de los tres | Por diseño, y en silencio [2026-09-14](#2026-09-14--sorpresas-al-escribir-las-lecciones-1-4) |
| `v new` sin entrada se detiene, o usa sus valores por defecto | Escribe `'<EOF>'` en `v.mod` como descripción, versión y licencia | `vlib/os/os.v` 436-441 con `cmd/tools/vcreate/vcreate.v` 170-181 | `v new hello < /dev/null`. La herramienta solo sustituye por su valor por defecto una respuesta *vacía*, y `'<EOF>'` no lo está | Reproducido; la CI lo sortea enviando tres respuestas [2026-09-14](#2026-09-14--instalación-de-v-052) |
| Aliasar un array y rebanarlo se comportan igual | `alias := original` comparte en silencio la memoria de un array `mut`; `first := lines[..2]` sobre el mismo array se clona, con un aviso | `vlib/v/checker/assign.v` 845-855 | El verificador solo busca rebanadas | Reproducido; la lección 4 muestra ambos [2026-09-14](#2026-09-14--sorpresas-al-escribir-las-lecciones-1-4) |
| Dos funciones con el mismo nombre siempre avisan de la redefinición | El error depende del sitio de la llamada | orden entre verificador y constructor, V 0.5.2 | `describe('Mission')` da `builder error: redefinition of function describe`; `describe(474)` solo da `cannot use int literal as string in argument 1 to describe` | Reproducido [2026-09-14](#2026-09-14--sorpresas-al-escribir-las-lecciones-1-4) |
| `fn [mut x]` captura por referencia | `spawn fn [mut total] (n int) { total += n }(i)` en un bucle compila y deja `total` en 0 | captura de closure, V 0.5.2 | La única pista es `warning: unused variable: total` sobre la lista de captura | Reproducido [2026-09-14](#2026-09-14--sorpresas-al-escribir-las-lecciones-5-8) |
| `v test` imprime lo mismo en local y en CI | Con `CI` o `GITHUB_JOB` puestos oculta las líneas `OK` salvo que `VTEST_HIDE_OK=0` | `cmd/tools/modules/testing/common.v` 37-43 | La primera pasada de CI de la lección 8 falló en los tres sistemas por eso | Reproducido [2026-09-14](#2026-09-14--la-ci) |
| Los números de un informe de test describen los datos | `Left value (len: 6): [9, 5]` da la longitud del texto impreso, y el resumen cuenta aserciones, no funciones de test | `v test -stats`, V 0.5.2 | `3 failed, 3 passed, 6 total` para tres funciones. Una función marcada `@[assert_continues]` cuyas aserciones fallaron aparece como `OK` | Reproducido; el código de salida sí es correcto [2026-09-14](#2026-09-14--sorpresas-al-escribir-las-lecciones-5-8) |
| `v doctor` indica el compilador de C que se usará | No siempre | `v doctor` frente a `v -showcc`, V 0.5.2 | `v doctor` dice `msvc version N/A` mientras `v -showcc -prod` compila con `cl.exe`. En macOS `v doctor` lista TCC mientras `v -showcc` muestra `cc` | Reproducido [2026-09-14](#2026-09-14--instalación-de-v-052) |
...
- [docs.vlang.io](https://docs.vlang.io/introduction.html) sigue `master`, no la versión publicada. Su página [The default compiler](https://docs.vlang.io/the-default-compiler.html) no está en el [`docs.md` de la 0.5.2](https://github.com/vlang/v/blob/0.5.2/doc/docs.md), y ni siquiera coincide con `master`: la página web (generada desde el commit `5db92f0`) dice que el ejecutable `v` "contains only the experimental V3 C compiler", mientras que [`doc/docs.md` en `master`](https://github.com/vlang/v/blob/7cbb67bec0a01138371f3a952f63cf8076fc1b1f/doc/docs.md) dice ahora que "contains the default compiler whose source lives in `vlib/v`". Las lecciones comprueban cada comportamiento con la propia 0.5.2.
