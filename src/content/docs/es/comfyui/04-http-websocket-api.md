...
- **Interrumpir.** `POST /interrupt` sin cuerpo detiene lo que se está ejecutando, lo haya encolado quien lo haya encolado; con `{"prompt_id": …}`, el código de la v0.36.0 solo lo detiene si ese prompt es el que se está ejecutando ([`server.py`, líneas 1163 a 1193](https://github.com/Comfy-Org/ComfyUI/blob/ee71d5c4993f29086b27fde1629a945ae48425bf/server.py#L1163-L1193)). Un prompt que no ha empezado se quita de la cola con `POST /queue` y `{"delete": [prompt_id]}`. Ambas responden `200` cuando no hay nada que detener y cuando el identificador no está en la cola: el código de estado dice que la llamada fue aceptada, no que hiciera algo. Para saberlo, hay que releer `/queue` y `/history` después. [`check.sh`](https://github.com/spareilleux/learn/blob/main/code/comfyui/check.sh) ejecuta ambas en los tres sistemas operativos.
- **Los errores durante la ejecución.** La validación ocurre antes del render, así que un prompt puede aceptarse y fallar igualmente. La comprobación sube un archivo que no es una imagen con un nombre en `.png`: `LoadImage` lo enumera, `POST /prompt` responde `200`, y el render termina con `execution_error: node 1 (LoadImage): cannot identify image file …`. Ambos clientes imprimen el nodo que lo lanzó y salen con código 1. Un worker debe distinguir eso de un fallo de red: volverá a fallar igual cada vez, así que reintentarlo desperdicia la cola. La lección 12 clasifica los fallos.
