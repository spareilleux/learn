...
- **`LoadImage` puede no enumerar el EXR que acaba de guardar, y eso depende de la máquina.** [El filtro](https://github.com/Comfy-Org/ComfyUI/blob/ee71d5c4993f29086b27fde1629a945ae48425bf/folder_paths.py#L229-L253) le pide a `mimetypes` de Python el tipo de cada archivo y conserva los que empiezan por `image`; y `mimetypes` lee la tabla del sistema operativo: el registro en Windows, archivos como `/etc/mime.types` en el resto. Medido por [`data/mime-info.py`](https://github.com/spareilleux/learn/blob/main/code/comfyui/data/mime-info.py) en tres máquinas, todas con Python 3.13: en esta máquina Windows y en el ejecutor `macos-latest`, `.exr` no tiene **ningún** tipo MIME, así que la lista lo descarta; en el ejecutor `ubuntu-latest` vale `image/aces`, y la lista lo conserva. El mismo ComfyUI, el mismo flujo, una lista distinta. `.glb` se reparte igual, y `.flac` vale `audio/flac` en ese Linux y `audio/x-flac` en las otras dos: ambos empiezan por `audio`, así que ese no cambia nada. Eso es lo que cuenta cuando se comparte un flujo: un archivo construido en una máquina donde `.exr` tiene un tipo MIME designa una imagen que la misma lista se niega a ofrecer en otro sitio; el flujo sigue siendo válido, pero ya no es reproducible. Si necesitas releer un EXR, pásalo por su ruta desde tu propio nodo, o añade el tipo a la máquina.
