...
- **int8**: 202 capas guardan un byte con signo por peso, más una escala `F32` por capa: el peso es aproximadamente la escala multiplicada por el byte. Cada una de estas capas tiene también un tensor `U8` diminuto llamado `comfy_quant`, que contiene su formato en JSON. El indicador `convrot` y su tamaño de grupo de 256 seleccionan una variante cuyos kernels, `quantize_int8_convrot_weight` y `dequantize_int8_convrot_weight`, están en [comfy-kitchen](https://github.com/Comfy-Org/comfy-kitchen), la biblioteca de cuantización de ComfyUI. El kernel ya se ha leído, en comfy-kitchen 0.2.34. La rotación es una matriz de Hadamard *regular*: las potencias de Kronecker de un bloque 4 × 4 simétrico, divididas por la raíz cuadrada de su tamaño, y por eso el tamaño de grupo tiene que ser una potencia de 4 ([`tensor/int8_utils.py`](https://github.com/Comfy-Org/comfy-kitchen/blob/v0.2.34/comfy_kitchen/tensor/int8_utils.py)). Cada grupo de 256 canales de entrada se rota con ella: el peso una vez, fuera de línea, como `W @ H.T`, y las activaciones en cada llamada, como `x @ H`, fusionado en el cuantizador por filas ([`backends/cuda/__init__.py`](https://github.com/Comfy-Org/comfy-kitchen/blob/v0.2.34/comfy_kitchen/backends/cuda/__init__.py)). Esa matriz es simétrica *y* ortogonal, así que es su propia inversa: las dos rotaciones se cancelan y la capa calcula el mismo producto. Lo que cambia es lo que el int8 tiene que contener. [`data/convrot.py`](https://github.com/spareilleux/learn/blob/main/code/comfyui/data/convrot.py) reconstruye la misma matriz —idéntica a la de comfy-kitchen con tamaños de grupo 16, 64 y 256— y cuantiza un peso en el que cada fila tiene un canal cuarenta veces mayor que los demás: el error de ida y vuelta por filas es del 5,7 % sin la rotación y del 0,76 % con ella, siete veces y media menor. La rotación mezcla el valor atípico con todo su grupo, de modo que la única escala de la fila ya no tiene que cubrirlo sola.
