...
`wslc`, su SDK de C# y Docker Desktop son todos software de otros, y `wslc` es lo bastante reciente como para que casi todo lo que sigue trate de la herramienta y no de los contenedores. Dos filas son documentación que no coincide con la versión publicada. Nada se ha reportado aguas arriba.

| Esperado | Lo que pasa | Dónde | Medición | Estado |
|---|---|---|---|---|
| El fragmento de `Microsoft.WSL.Containers` de Microsoft Learn compila contra el paquete | Cinco errores de compilación: `ComponentFlags`, `SessionSettings.MemoryMB`, `ProcessSettings.CmdLine` y `DeleteContainerFlags` no existen. El paquete los llama `IReadOnlyList<Component>`, `MemorySizeInMB`, `CommandLine` y `DeleteContainerOption` | `Microsoft.WSL.Containers` 2.9.9 | Cinco pares de nombres, documentación contra paquete | Reproducido, no reportado [2026-09-13](#2026-09-13--un-programa-c-con-microsoftwslcontainers) |
| El framework de destino del paquete coincide con el SDK contra el que se construyó | `wslcsdkcs.dll` se construyó contra `Microsoft.Windows.SDK.NET` 10.0.26100.79, que no está en nuget.org — la más cercana es 10.0.26100.80 — de ahí un CS1705 al referenciarlo | `Microsoft.WSL.Containers` 2.9.9 | CS1705, y una versión de SDK imposible de restaurar | Reproducido, sorteado fijando `WindowsSdkPackageVersion` [2026-09-13](#2026-09-13--un-programa-c-con-microsoftwslcontainers) |
| Un contenedor se limpia cuando `Start()` lanza | Sobrevive en `storage.vhdx`, y el siguiente `Start()` con el mismo nombre falla | `Microsoft.WSL.Containers` 2.9.9 | `COMException`, «name already in use» | Reproducido, sorteado con un `Delete(Force)` en `finally` y una limpieza al arrancar [2026-09-13](#2026-09-13--un-programa-c-con-microsoftwslcontainers) |
| Un contenedor creado por la API tiene acceso a la red, como se lo da `wslc run` | Sin `ContainerSettings.NetworkingMode` obtiene `NetworkMode: none` y ningún acceso; necesita un `Bridged` explícito | `Microsoft.WSL.Containers`, 2.9.11 | `ip -4 addr` y `wget` antes y después | Reproducido, valor por defecto defendible, no documentado [2026-09-13](#2026-09-13--ir-más-allá-wslcimage-red-kubernetes-interfaz-gráfica) |
| `wslc` tiene un subcomando Compose, como `docker compose` | `Unrecognized command: 'compose'` — no hay ninguno en 2.9.11 | `wslc` 2.9.11 | `wslc --help` | Reproducido; el curso escribe su propio lanzador en su lugar [2026-09-13](#2026-09-13--compose-con-wslc) |
| `wslc image prune -f` fuerza, como el `-f` de `docker` | `-f` significa `--filter`, y `--force` no existe | CLI de `wslc` 2.9.11 | Una bandera que se lee como `--force` y toma un filtro | Reproducido, trampa que la lección conserva [2026-09-13](#2026-09-13--preguntas-abiertas-restantes) |
| El origen de un montaje enlazado se lee como una ruta dentro del contenedor | Se interpreta como una ruta de Windows: `-v /var/run/docker.sock:/var/run/docker.sock` crea así en silencio una carpeta vacía real `C:\var\run\docker.sock\` | `wslc` 2.9.11 | Un directorio creado en el anfitrión, sin error | Reproducido, no reportado [2026-09-13](#2026-09-13--compose-con-wslc) |
| Borrar imágenes y terminar una sesión devuelve espacio en disco | `storage.vhdx` solo crece | `%LocalAppData%\wslc\sessions\<sesión>\storage.vhdx` | 814 → 1070 → 1550 MB a lo largo de un pull, un remove, un prune, un terminate y otro pull | Reproducido; `Optimize-VHD -Mode Full` recupera parte, 3995 → 2789 MB en 10 s [2026-09-13](#2026-09-13--preguntas-abiertas-restantes) |
| Dos motores que publican el mismo puerto del anfitrión chocan, o al menos avisan | `wslc` y Docker Desktop toman ambos el 8080 en silencio, y a cuál llegas depende del nombre que uses | red de Windows, `wslc` 2.9.11 frente a Docker Desktop 4.61 | `127.0.0.1` llegó siempre a `wslc` y `localhost` siempre a Docker, en las cuatro variantes de orden de arranque y dirección explícita | Reproducido, ningún error en ninguna variante [2026-09-13](#2026-09-13--preguntas-abiertas-restantes) |
| `ContainerSettings.Privileged = true` da un contenedor privilegiado, como `docker run --privileged` | No cambia nada: capacidades, cgroups y `mount` vuelven idénticos a `Privileged = false`, también en una sesión de administrador. `wslcsdk.h` declara `WSLC_CONTAINER_FLAG_PRIVILEGED = 0x4`, que no se aplica | `wslc` 2.9.11 y su SDK | Máscaras `CapEff` y tablas de montaje de cgroups, antes y después; k3s sigue muriendo con `failed to evacuate root cgroup: read-only file system` | Reproducido, no reportado. Por eso no hay Kubernetes sobre `wslc` [2026-09-13](#2026-09-13--ir-más-allá-wslcimage-red-kubernetes-interfaz-gráfica) |

## Experimentos

Siete preguntas que el curso midió en lugar de suponer. Donde la hipótesis era una línea de documentación — los comentarios del propio `settings.yaml`, por ejemplo — la fila la cita, porque era eso lo que se ponía a prueba.

| Pregunta | Hipótesis | Resultado | Veredicto | Dónde |
|---|---|---|---|---|
| ¿Cambian `cpuCount` y `memorySize` de `settings.yaml` lo que ve un contenedor? | Los comentarios del propio archivo lo dicen: «Number of virtual CPUs allocated to the session… default: all» y «Memory limit… default: half of available memory» | Por defecto: 24 CPU y 31 946 MB. Con `cpuCount: 4, memorySize: 4GB`: `nproc` 4, 3919 MB de RAM y 4096 MB de swap | Confirmada | [2026-09-13](#2026-09-13--limitar-la-cpu-y-la-memoria-de-una-sesión-wslc) |
| ¿Alcanza una edición de `settings.yaml` a una sesión ya arrancada? | Implícita: un ajuste guardado surte efecto | La primera medición tras la edición seguía mostrando 24 CPU; hay que terminar la sesión antes | Refutada | [2026-09-13](#2026-09-13--limitar-la-cpu-y-la-memoria-de-una-sesión-wslc) |
| ¿Cuándo se desmonta la VM de una sesión inactiva? | Al `idleTimeout` configurado, 30 s | En marcha a los 0 s, ausente a los 35 s, consultando cada 5 s: entre 30 y 35 s | Confirmada | [2026-09-13](#2026-09-13--limitar-la-cpu-y-la-memoria-de-una-sesión-wslc) |
| ¿Es `memorySize` una reserva o un techo, y se devuelve la memoria al parar un contenedor? | No escrita como predicción; la tabla era el objeto mismo | VM en reposo unos 921 MB. Sujetando 6 GB: 7069 MB. Diez segundos tras parar y borrar: 2776 MB. Más tarde: 902 MB | Un techo, no una reserva, y devuelta poco a poco en lugar de de golpe | [2026-09-13](#2026-09-13--limitar-la-cpu-y-la-memoria-de-una-sesión-wslc) |
| ¿Recupera `Optimize-VHD` lo que `storage.vhdx` no devuelve? | No escrita como predicción | 3995 MB a 2789 MB en 10 s — todavía por encima de los 1,9 GB realmente ocupados dentro | En parte: recupera algo, no todo | [2026-09-13](#2026-09-13--preguntas-abiertas-restantes) |
| ¿Pueden `wslc` y Docker Desktop publicar ambos el puerto 8080, y qué cliente llega a cuál? | No escrita como predicción: la pregunta era si siquiera se permitiría | Ningún error en las cuatro variantes. `127.0.0.1` llegó a `wslc` siempre, `localhost` a Docker siempre | Confirmada, y conviene saberlo | [2026-09-13](#2026-09-13--preguntas-abiertas-restantes) |
| ¿Restringen las opciones `--memory` y `--cpus` por contenedor al contenedor o a la VM? | Escrita de antemano: deberían aparecer en la vista del propio contenedor | `memory.max=536870912` y `cpu.max=150000 100000` coinciden exactamente con las opciones, pero `nproc` sigue diciendo 8 y `free -m` sigue diciendo 15 996 MB | Confirmada en el cgroup, refutada en lo que ve el contenedor | [2026-09-16](#2026-09-16--lote-2-cinco-lecciones-nuevas) |
