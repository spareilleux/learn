...
| `PCFSoftShadowMap` funciona en `WebGPURenderer` | Retirado en r186, y avisa. R3F lo pone para cualquier `shadows` verdadero y recibe el mismo aviso en cada render de `<Canvas>` | three.js r186, @react-three/fiber 9.7.0 | `LunarLanderEngine.ts` y `MinimalThreeInstrument.tsx` de GA lo siguen combinando con `WebGPURenderer` | Reproducido [2026-09-16](#2026-09-16--sorpresas) · [2026-09-16](#2026-09-16--lecciones-9-a-13-lo-que-encontraron-las-páginas) · Todavía no en GA, que fija three 0.180.0, donde sigue funcionando; [2026-09-24](#2026-09-24--correcciones-upstream) |
...
- La fila `PCFSoftShadowMap` todavía no se aplica a GA. La retirada y su aviso son reales en r186 (`Renderer.js` y `ShadowNode.js` del paquete `three@0.186.0`), pero GA fija three 0.180.0 y @react-three/fiber 8.18.0. En 0.180, `ShadowNode` todavía asocia `PCFSoftShadowMap` a su filtro suave: no hay aviso, y las sombras suaves funcionan.
- Los dos emparejamientos de GA con `WebGPURenderer`, en `LunarLanderEngine.ts` y `MinimalThreeInstrument.tsx`, empezarán a avisar cuando GA pase a r186 o posterior. Cambiarlos ahora haría perder las sombras suaves, así que [#733](https://github.com/GuitarAlchemist/ga/pull/733) los deja para esa actualización y lo dice. Leído en las fuentes del paquete, sin ejecutar (*por verificar*).
