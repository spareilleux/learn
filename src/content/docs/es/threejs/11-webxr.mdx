...
[Abrir a pantalla completa](../../../threejs-demos/demos/xr.html?emulate). **Pruébalo**: esta página no muestra nada sin la sonda; la [demo en vivo](#demo-en-vivo-la-guitarra-en-vr) al final de la lección entra en la misma sesión emulada con un botón y responde a los ejercicios 1 y 3.
...
## Demo en vivo: la guitarra en VR

Los ejercicios 1 y 3 sobre una guitarra entera: el `WebGLRenderer` clásico, el que esta lección vio renderizar los dos ojos con IWER, un `VRButton` y un manejador de `select` que lleva el rayo del mando a las coordenadas de la guitarra, encuentra la cuerda y el traste como la guitarra de la lección 5, toca la nota y envía un pulso háptico de 40 ms. IWER solo se importa con `?emulate`, así que un visor real no descarga nada de él. La página ignora `resize` durante una sesión: `WebGLRenderer` se niega a cambiar de tamaño mientras presenta, y avisa. Su sonda entra en una sesión emulada, apunta el mando derecho a la cuerda 3, traste 5, aprieta el gatillo y lee `string 3, fret 5: C4`, con 2 ojos y ningún error. El [código](https://github.com/spareilleux/learn/tree/f68e710/code/threejs/demos/xr) está en `code/threejs/demos/xr`.

![Dos vistas casi idénticas lado a lado, el ojo izquierdo y el derecho de un visor emulado: una guitarra eléctrica entera flotando sobre un suelo de cuadrícula gris, vista desde arriba, con un fino rayo de mando naranja que cruza sus cuerdas en cada vista.](../../../../assets/threejs/demo-xr.webp)

<details>
<summary>Página en vivo: ejecutarla aquí</summary>

<iframe src="../../../threejs-demos/demos/xr.html?emulate" title="La guitarra en VR" loading="lazy" width="100%" height="420" allow="fullscreen; autoplay; xr-spatial-tracking" style={{ border: 0, display: "block", height: "min(420px, 70vh)" }}></iframe>

</details>

[Abrir a pantalla completa](../../../threejs-demos/demos/xr.html?emulate). **Pruébalo**: pulsa *Enter VR*: el mando derecho emulado toca entonces solo un arpegio de do mayor. Con un visor, abre la página sin `?emulate` en su navegador, apunta al mástil y aprieta el gatillo; *por verificar*, el autor no tiene visor.

## Experimento GA

El [experimento 12](../14-guitar-alchemist-lab/#12-webxr-a-escala-11) de la lección 14 coloca un mástil a tamaño real en un Quest 3 emulado: dos vistas a 63 mm una de otra, y la distancia de la cejuela al traste 12 tal como se midió. La sensación en la mano, y el desfase de cabeza de 25 mm de IWER, están *por verificar* en un visor.
