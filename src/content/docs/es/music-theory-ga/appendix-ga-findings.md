...
|---|---|---|---|---|
| 1 | [`Pitch.Flat.DFlat/FFlat/GFlat(Octave)`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Domain.Core/Primitives/Notes/Pitch.cs#L285-L295) construyen la nota equivocada: cada uno devuelve la nota de la línea siguiente | 3 | 1 | **Corregido** (fábricas corregidas; `EFlat`/`BFlat` añadidos) |
| 2 | La regex de [`PitchParser`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Domain.Core/Primitives/Notes/PitchParser.cs#L20-L25) no tiene anclas e ignora las mayúsculas, así que `Eb2` coincide como `b2` = B2 | 1 | 1 | **Corregido** (regexes ancladas precompiladas estáticas) |
| 3 | [`Note.Flat.TryParse`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Domain.Core/Primitives/Notes/Note.cs#L219-L237) pasa a mayúsculas antes de sustituir `♭`, así que `B` se convierte en B♭ y `D♭` se rechaza | 1 | 1 | **Corregido** (`B` natural analizado en B; Unicode `♭` gestionado) |
| 4 | [`PitchClass.TryParse`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Domain.Core/Theory/Atonal/PitchClass.cs#L252-L268) prueba los alias hexadecimales `A`=10, `B`=11 antes que los nombres de notas | 1 | 1 | **Corregido** (`TryParseSetNotation` separado de `TryParse`) |
| 5 | [`SimpleIntervalSize.TryParse`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Domain.Core/Primitives/Intervals/SimpleIntervalSize.cs#L155-L164) y `CompoundIntervalSize.TryParse` lanzan una excepción en lugar de devolver `false` | 1 | 1 | **Corregido** (devuelve `false` con entrada no válida) |
| 6 | [`ModalFamily`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Domain.Core/Theory/Atonal/ModalFamily.cs#L114-L128) agrupa los conjuntos por vector interválico pero llama `Modes` a sus miembros | 2 | 2 | Diseño conocido (agrupación por ICV) |
| 7 | [`ChordFormula.DetermineQuality`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Domain.Core/Theory/Harmony/ChordFormula.cs#L172-L214) nunca devuelve `Major7`, `Minor7`, `HalfDiminished` ni `Diminished7` | 2 | 3 | **Corregido** (cualidades y sufijos de `ChordFormula`) |
| 8 | [`DetermineExtension`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Domain.Core/Theory/Harmony/ChordFormula.cs#L216-L294) lee nueve semitonos como una sexta mayor, nunca como una séptima disminuida | 1 | 3 | **Corregido** (séptima disminuida distinguida de sexta/13ª) |
| 9 | [`Note.Chromatic.ToAccidented`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Domain.Core/Primitives/Notes/Note.cs#L75-L77) es `ToSharp().ToAccidented()`: cada tecla negra recibe un nombre con sostenido | 8 | 3 | **Corregido** (transposición con ortografía correcta en `Chord`) |
| 10 | [`Chord`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Domain.Core/Theory/Harmony/Chord.cs#L241-L250) rota su lista de notas para una inversión y luego la reanaliza desde el bajo como si fuera la fundamental | 1 | 3 | **Corregido** (`Root` y `Formula` originales conservados) |
| 11 | [`Voicing.HasBarre`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Domain.Core/Instruments/Fretboard/Voicings/Core/Voicing.cs#L73-L80) cuenta el traste 0, así que las cuerdas al aire forman una cejilla | 3 | 3 | **Corregido** (filtro `fret > 0` en `Voicing.HasBarre()`) |
| 12 | [`IntervalClassVectorId`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Domain.Core/Theory/Atonal/IntervalClassVectorId.cs#L39-L85) empaqueta seis recuentos como dígitos en base 12, y un recuento de 12 desborda su dígito | 1 | 4 | Límite fijado (conserva el orden del catálogo) |
| 13 | [`KeyTools.GetParallelKey`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/GaMcpServer/Tools/KeyTools.cs#L161-L166) tiene el mismo cuerpo que `GetRelativeKey`: conserva la armadura en lugar de la tónica | 5 | 5 | **Corregido** (conserva la tónica con modo opuesto) |
| 14 | [`KeyTools.GetNeighboringKeys`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/GaMcpServer/Tools/KeyTools.cs#L204) busca una tonalidad por sus alteraciones en lugar de por su nombre, y lanza una excepción | 1 | 5 | **Corregido** (posición en círculo de quintas por recuento de armadura) |
| 15 | [`Key.GetInterval`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Domain.Core/Theory/Tonal/Key.cs#L76) pasa sus argumentos al revés y devuelve la inversión | 2 | 5 | **Corregido** (orden tónica-a-nota restablecido) |
| 16 | [`Key.Major.TryParse`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Domain.Core/Theory/Tonal/Key.cs#L155-L158) lanza una excepción ante una fundamental no analizable aunque tiene un camino `return false` | 1 | 5 | **Corregido** (devuelve `false` con fundamental no analizable) |
| 17 | [`HarmonicFunction.FromDegree`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Domain.Core/Theory/Tonal/HarmonicFunction.cs#L32) solo ve un número de grado, y su enum no tiene `Subtonic` | 1 | 6 | **Corregido** (`Subtonic` añadido con comprobación de semitonos) |
| 18 | [`Cadences.yaml`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Business.Config/Cadences.yaml#L124-L129) numera una fila en tonalidad menor desde el mayor homónimo (`♭iii` para G en E menor) | 1 | 7 | **Corregido** (corregido a `iii` en Cadences.yaml) |
| 19 | [`PitchClassSet.ClosestDiatonicKey`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/Common/GA.Domain.Core/Theory/Atonal/PitchClassSet.cs#L609-L615) resuelve el empate mayor/menor con una forma normal, que no puede codificar un modo | 4 | 7 | Heurística conocida (ambigüedad intrínseca de tónica del PC-set) |
