...
Un conjunto de clases de altura también se puede dibujar. Coloca las doce clases de altura en la esfera de un reloj, 0 arriba y las demás en el sentido de las agujas del reloj, rellena las que pertenecen al conjunto y únelas: Ian Ring llama a esta figura **diagrama de brazalete** (*bracelet diagram*), que "shows tones that are in this scale, starting from the top (12 o'clock), going clockwise in ascending semitones" ([escala 2741](https://ianring.com/musictheory/scales/2741)). La escala mayor rellena 0, 2, 4, 5, 7, 9 y 11; los dos lugares donde dos cuentas llenas son vecinas, 4 y 5, 11 y 0, son los dos semitonos del patrón.

<BraceletMajor role="img" aria-label="Diagrama de brazalete de la escala mayor: clases de altura 0 2 4 5 7 9 11 rellenas sobre una esfera de reloj de 12, unidas por un polígono." />

GA tiene un componente React para la misma figura, [`BraceletNotation`](https://github.com/GuitarAlchemist/ga/blob/a826864f3a012cad88e415954bf57eca0ce12aa6/ReactComponents/ga-react-components/src/components/Atonal/BraceletNotation.tsx#L17-L45), que toma el número de 12 bits, lee el bit *i* como la clase de altura *i* y dibuja una cuenta cada 30 grados. El curso dibuja sus propios diagramas con [`Diagrams.cs`](https://github.com/spareilleux/learn/blob/a2439ba/code/music-theory-ga/GaTheory/Diagrams.cs), con los mismos números que las tablas, y la CI comprueba que las imágenes del repositorio están al día.
...
En el brazalete, el desplazamiento circular es una rotación: cada cuenta avanza dos posiciones en el sentido de las agujas del reloj, y el polígono de D mayor es el polígono de C mayor girado 60 grados.

<BraceletTransposition role="img" aria-label="Dos brazaletes. T0, la escala mayor sobre C: 0 2 4 5 7 9 11. T2, la escala mayor sobre D: 1 2 4 6 7 9 11, el mismo polígono girado dos posiciones en el sentido de las agujas del reloj." />
...
Los brazaletes muestran la diferencia. El dórico sobre C es el polígono de la escala mayor girado dos posiciones en sentido contrario a las agujas del reloj, de modo que la cuenta de D cae en 0. Todos los modos de la escala mayor son este mismo polígono; lo que cambia es cuál de sus cuentas queda arriba, la tónica.

<BraceletModes role="img" aria-label="Dos brazaletes con el mismo polígono. 2741, jónico: 0 2 4 5 7 9 11. 1709, dórico sobre C: 0 2 3 5 7 9 10, el primer polígono girado dos posiciones en sentido contrario a las agujas del reloj." />
...
Los brazaletes de las dos escalas simétricas explican sus recuentos. La escala de tonos enteros es un hexágono regular: gíralo dos posiciones y se cubre a sí mismo, así que toda rotación que pone una cuenta arriba da el mismo conjunto, un modo. La escala octatónica se cubre a sí misma tras tres posiciones, así que sus rotaciones solo dan dos conjuntos distintos.

<BraceletSymmetric role="img" aria-label="Dos brazaletes. 1365, la escala de tonos enteros: 0 2 4 6 8 10, un hexágono regular. 2925, la escala disminuida u octatónica: 0 2 3 5 6 8 9 11, una forma que se repite cada tres semitonos." />
...
Las otras seis escalas de la tabla son su propia imagen especular y no tienen pareja relacionada por Z, así que para ellas los dos recuentos coinciden. Una clase de vector interválico es un objeto real de la teoría de conjuntos, pero no es lo que significa *modo*. Un modo es una rotación de una escala, y una escala tiene tantos modos como notas, salvo que una simetría rotacional haga que dos rotaciones den el mismo conjunto: "Modes are the rotational transformations of this scale. This number includes the scale itself, so the number is usually the same as its cardinality" ([Ian Ring, escala 2477](https://ianring.com/musictheory/scales/2477)). Sus páginas responden 7 para [la menor armónica](https://ianring.com/musictheory/scales/2477) y 6 para [la escala de blues](https://ianring.com/musictheory/scales/1257), las cifras del curso. El propio comentario de clase de `ModalFamily` dice lo que de verdad construye, conjuntos de clases de altura "that share the same interval vector", pero los llama `Modes`, y quien cuente en GA los modos de la menor armónica obtiene el doble.
