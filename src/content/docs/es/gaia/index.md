---
title: Gaia — Misión
description: Gaia es un bus de coordinación local y duradero y una fábrica de software que aporta evidencias para sesiones de Claude Code y Codex — seis verbos sin privilegios, un registro de eventos solo de anexión y recibos que nadie tiene que creerse bajo palabra. Este curso explica qué intenta conseguir, por qué el privilegio se impide por ausencia y no mediante una comprobación, y qué se ha medido realmente, con cada salida ejecutada sobre la revisión fijada.
sidebar:
  label: Misión
  order: 0
---
...
Gaia en el commit [`d68e900`](https://github.com/GuitarAlchemist/gaia/tree/d68e90099ae2a6fbbec9d428617bfcd02095aac0) (2026-09-13), la cabeza de `main`, en Windows 11 con [Node.js](https://nodejs.org/en) v24.12.0, el 2026-09-15. Gaia tiene **cero dependencias en tiempo de ejecución**, así que cada comando de este curso se ejecuta desde un checkout limpio sin `npm install`.

Cada salida citada en estas lecciones se produjo ejecutando el comando sobre esa revisión, en un directorio de datos desechable, y se pegó sin editar, salvo el acortamiento de las rutas absolutas largas. **Ninguna lección de este curso lanza un modelo de pago.** El único comando que lo hace — `factory:agent`, que gasta un turno real de Claude y un turno real de Codex — se describe a partir de su código, de su documento de diseño y del esquema de su recibo, y está marcado *por verificar* allí donde no lo he ejecutado.
:::
