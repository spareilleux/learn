...
| Un trabajo de la fábrica termina tras su worker | El trabajo de la issue 108 seguía en `STARTED` unas diez horas después de terminar su worker, ocupando el único hueco del host | `C:/Gaia/state`, `portfolio:autonomous status` | Worker terminado a las 00:25 hora local, trabajo `STARTED` a las 10:00 | *Por verificar* |
...
- **El trabajo de la fábrica en la issue 108.** Su worker terminó a las 00:25 hora local, pero el trabajo seguía en `STARTED` unas diez horas después, ocupando el único hueco del host, así que la Draft de la issue 104 espera. Averiguar si el proceso `watch` se detuvo o si el paso de revisión nunca empezó, y qué reconciliación hace un reinicio.
