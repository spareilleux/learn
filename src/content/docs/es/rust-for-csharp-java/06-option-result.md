...
Rust **no tiene `null`** ni **excepciones**. Ambos se sustituyen por dos enums de la biblioteca estándar — construidos exactamente como los de la [lección 5](../05-structs-enums-match/) ([`core/src/option.rs`](https://github.com/rust-lang/rust/blob/1.94.0/library/core/src/option.rs#L600-L609), [`core/src/result.rs`](https://github.com/rust-lang/rust/blob/1.94.0/library/core/src/result.rs#L557-L567)):
...
Los dos caminos que puede seguir un valor a través del operador de interrogación.

```mermaid
flowchart LR
    expr["expr?"] --> check{"¿Ok o Some?"}
    check -->|"sí"| value["extraer el valor y continuar"]
    check -->|"no"| ret["devolver el Err o el None desde la función actual"]
```
...
Un error puede ser de cualquier tipo; un `enum` enumera las formas en que una operación puede fallar ([líneas 32-60](https://github.com/spareilleux/learn/blob/93f6f82/code/rust-for-csharp-java/examples/l06_option_result.rs#L32-L60)):
