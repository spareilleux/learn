...
- **`null` se rechaza.** La [especificación Reactive Streams](https://github.com/reactive-streams/reactive-streams-jvm/blob/master/README.md#2.13) prohíbe los elementos `null`, así que un mapper que devuelve `null` falla. `Mono.justOrEmpty` convierte un valor posiblemente nulo en un `Mono` vacío, igual que `?.` y `??` gestionan `null` en C#.
...
- [Especificación Reactive Streams](https://github.com/reactive-streams/reactive-streams-jvm/blob/master/README.md), [`java.util.concurrent.Flow`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/Flow.html)
