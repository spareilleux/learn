...
three.js r186 and the libraries around it are somebody else's software; this table holds what the lessons found in them, not in the pages the course writes. Most of it comes from the move to `WebGPURenderer`, which is new enough that a WebGL habit is often the thing that is wrong. Nothing here has been reported upstream except where the Status column names an existing issue.
...
| Compression keeps every vertex the geometry has | Draco and meshopt both drop the two pole vertices `SphereGeometry` creates and no triangle uses; Draco's `weld()` also merges the non-indexed body's corners | Draco, meshopt, glTF Transform 4.5.0 | Vertex counts before and after, in `expected/` | Reproduced, defensible [2026-09-16](#2026-09-16--surprises) |
| drei's `<Text>` renders on `WebGPURenderer` as it does on WebGL | It draws a blank quad: troika-three-text injects its shader through `onBeforeCompile`, which the node renderer never calls | drei 10.7.8 | Blank quad | Reproduced. Still open: whether a WebGPU-ready troika-three-text exists [2026-09-16](#2026-09-16--lessons-9-to-13-what-the-pages-found) |
| `setColorAt` colours instances whatever the first render did | An `InstancedMesh` ignores it if its first render had no instance colors | three.js `InstancedMesh` | The port's markers stayed white until the attribute was allocated up front | Reproduced, worked around [2026-09-16](#2026-09-16--lessons-9-to-13-what-the-pages-found) |
| WebXR on `WebGPURenderer` behaves as on `WebGLRenderer` | It needs `XRGPUBinding` and the `webgpu` feature; its error message names a `VRButtonGPU` that does not exist; `WebGLXRFallback` swaps renderers but the controllers taken from the first one stop receiving events | three.js r186 WebXR on WebGPU | Only the classic `WebGLRenderer` rendered both eyes under IWER | Reproduced [2026-09-16](#2026-09-16--lessons-9-to-13-what-the-pages-found) |
| `InstancedMesh` works with morph targets | It fails with one instance, `morphTargetInfluences` never created, and differently with several, in `UniformArrayNode.updateBuffer`. It also needs `geometry.morphTargetsRelative = true`, silently | three.js r186 | Two distinct failures, one per instance count | Reproduced [2026-09-17](#2026-09-17--lesson-14-the-guitar-alchemist-3d-lab) |
| `BatchedMesh` batches into few draw calls on every backend | On WebGPU it draws one call per object | three.js r186 `BatchedMesh`, WebGPU backend | 44 draw calls for one neck against 3 on WebGL 2, which uses `WEBGL_multi_draw` | Reproduced [2026-09-17](#2026-09-17--lesson-14-the-guitar-alchemist-3d-lab) |
| `renderer.info.memory.texturesSize` counts compressed textures | It counts 0 bytes for KTX2 textures, and counts render targets instead | three.js r186 `renderer.info.memory` | 0 bytes for the compressed wood set, which is why experiment 6 could not be finished | Reproduced. Still open: measuring KTX2 GPU memory with a GPU tool [2026-09-17](#2026-09-17--lesson-14-the-guitar-alchemist-3d-lab) |
| `powerPreference` picks a GPU | Chromium on Windows ignores it and warns; the other OSes do not | Chromium 153 | `check.sh` filters the line | Known upstream, Chromium issue 369219127 [2026-09-16](#2026-09-16--lessons-9-to-13-what-the-pages-found) |
| three.js's `RapierPhysics` addon follows a current Rapier | r186's addon still loads 0.17.3 from skypack.dev, while the course uses 0.20.0 | three.js r186 `examples/jsm/physics/RapierPhysics` | Two minor versions apart | Reproduced, the course does not use the addon [2026-09-16](#2026-09-16--lessons-9-to-13-what-the-pages-found) |
| R3F's WebGPU build ships the WebGPU entry point alone | It imports `three` next to `three/webgpu`, so the WebGL core is bundled as well | @react-three/fiber 9.7.0 | 565 kB gzip against 210 kB for the same scene without React | Reproduced. Still open: aliasing `three` to `three/webgpu` [2026-09-17](#2026-09-17--lesson-14-the-guitar-alchemist-3d-lab) |

## Experiments

Lesson 14 ran fifteen experiments in the Guitar Alchemist 3D lab. Each hypothesis and its numeric predictions were committed on 16 September, the day before the measurements, which is what lets a row below say *refuted*. Of the 61 predictions, 19 were wrong in whole or in part and 2 could not be measured. Measured on an RTX 5080 in Chromium 153, GPU times from WebGPU timestamp queries. Detail in [lesson 14](#2026-09-17--lesson-14-the-guitar-alchemist-3d-lab).

| Question | Hypothesis | Result | Verdict |
|---|---|---|---|
| Which way of drawing 16 guitar necks costs least: GA's own, instanced, batched or merged? | Draw calls drive CPU time; the GPU barely moves between them | Batched on WebGPU drew 44 calls, not the 4 or fewer predicted. GA's 16 necks cost 2.9 ms of CPU, not the 10 ms or more predicted | Mostly refuted |
| Rewriting a vibrating string's vertices on the CPU against doing it in TSL, up to 600 strings | At 600 strings the CPU rewrite costs more, and the GPU cost is about the same | CPU 1.8 ms against TSL 0.36 ms, a factor of 5, as predicted. But TSL cost 1.7 times the GPU time of the CPU version | Confirmed on the CPU, refuted on the GPU |
| Picking one note among 10,000: `Raycaster` against a GPU ID pass | The GPU pick wins at 10,000 objects | `Raycaster` 0.465 ms against 3.395 ms for the GPU pick | Refuted |
| Transposing 49,152 pitch-class bracelets, on the CPU against TSL | The CPU pays for rebuilding 49,152 matrices a frame: 4 ms or more | CPU 2.455 ms, below the prediction. TSL 0.29 ms, as predicted | Confirmed on TSL, refuted on the CPU |
| An OPTIC-K point cloud up to 1,000,000 points, and whether TSL compute needs WebGPU | Points stay cheap to a million; the compute shader does not run on WebGL 2 | Points held. The compute shader did run on WebGL 2, through transform feedback, at 2.8 GPU ms | Confirmed on the points, refuted on the backend |
| What the wood textures cost uncompressed, and what KTX2 saves | RGBA8 costs about 22 MB; KTX2 in BC7 costs about a quarter of it | 21.33 MB confirmed. The KTX2 figure could not be read: `renderer.info` counts 0 for compressed textures | Half confirmed, half not measurable |
| A grid of PBR materials: how many shader programs, and what `MeshPhysicalMaterial` adds | One program per material type, shared across the grid; the physical material costs more GPU time | 16 programs either way, as predicted. The physical material cost 1.46 times the GPU time | Confirmed |
| What a dynamic shadow map costs against a baked one | A dynamic shadow map roughly doubles the draw calls | 13 draw calls against 8, a factor of 1.6. The bake took 398 ms, as predicted | Roughly confirmed |
| What bloom costs at 1080p | Between 0.5 and 1.5 GPU ms | 0.135 ms, four times below the lowest prediction | Refuted |
| A wall of 1,000 guitars with LOD and frustum culling | LOD divides the triangles by about 10 | By 11.2 | Confirmed |
| Whether Rapier's CCD stops a kinematic pick going through a string | CCD prevents the pass-through | The pick passed through at 30 and at 60 steps a second, CCD on or off | Refuted |
| Whether a fretboard is at 1:1 scale in WebXR, under IWER emulation | Nut to twelfth fret measures 0.324 m | 0.324 m | Confirmed |
| How far ahead a note has to be scheduled for WebAudio and the frame to agree | The naive lead is about the sum of the latencies, 50 ms, within one frame | 31.8 ms, 18 ms short of the sum | Refuted |
| What React Three Fiber costs against vanilla three.js, for the same scene | Cheap per frame, costly at startup (+30 to 150 ms) and in the bundle (+50 to 90 kB) | Cheap per frame, as predicted. First frame +430 ms and bundle +195 kB, both far above | Confirmed per frame, refuted on both costs |
| Whether an adaptive pixel ratio settles on a weak GPU | GPU time is proportional to the pixel count, and the controller settles within 60 frames | It settled at 0.655 within 60 frames | Confirmed |
