...
import patchesShot from '../../../assets/threejs/l03-patches-aces.webp';
import environmentShot from '../../../assets/threejs/l03-environment.webp';
...
<img src={patchesShot.src} width={patchesShot.width} height={patchesShot.height} alt="Two rows of gray squares on a dark background. The top row goes from dark gray to white in six steps; the bottom row has four squares alternating a middle gray and a lighter gray." loading="lazy" style={{ maxWidth: "100%", height: "auto" }} />
...
<img src={environmentShot.src} width={environmentShot.width} height={environmentShot.height} alt="Ten tan spheres in two rows against a blurred gray studio background. In the top row, the plastic-like spheres go from glossy, with a sharp white reflection of a softbox, to matte. In the bottom row, the metal spheres go from a mirror-like copper with a sharp reflection to a soft, evenly lit copper." loading="lazy" style={{ maxWidth: "100%", height: "auto" }} />
