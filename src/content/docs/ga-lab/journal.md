...
| The same commit exports the same index | Deduplication keeps the cheapest voicing of each group, generation runs in parallel, and ties go to whichever arrives first | [`OptickIndexWriter.cs`](https://github.com/GuitarAlchemist/ga/blob/66bdd049ad3f4b10f996e4cc6a9c6e58797cf30e/Demos/Music%20Theory/FretboardVoicingsCLI/OptickIndexWriter.cs) | Two exports of one build: every shared vector identical, 70 voicings present in one and missing from the other | Fixed upstream by [#730](https://github.com/GuitarAlchemist/ga/pull/730), merged 2026-09-24: generation is still parallel, but the voicings come out in fret order, so ties go to the same voicing every run. Two guitar exports of GA's main hashed `776c849e…` and `d0a7d1d9…` before, `138603e8…` both times after |
| `--export-max N` samples the index | It keeps the first N raw voicings in generation order, which starts high on the neck | [`OptickIndexWriter.cs`](https://github.com/GuitarAlchemist/ga/blob/66bdd049ad3f4b10f996e4cc6a9c6e58797cf30e/Demos/Music%20Theory/FretboardVoicingsCLI/OptickIndexWriter.cs) | `--export-max 3000` produced 2,641 vectors, 0 of them in the full index | Changed by [#730](https://github.com/GuitarAlchemist/ga/pull/730): the first N voicings are now the same on every run and come from the nut up, in fret order; they are still not a sample of the index. Not re-measured (*to verify*) |
...
- The two `ga_chord_to_set` rows of P5 are fixed upstream by [#681](https://github.com/GuitarAlchemist/ga/pull/681), merged 2026-09-23: the F# closure behind the tool now applies the alterations and implies the seventh. Re-run on GA [`27a1257`](https://github.com/GuitarAlchemist/ga/commit/27a1257f7fe5478c4b1bcb6b86ebe101707ac935) by music-theory-ga's [`recheck/probe.cs`](https://github.com/spareilleux/learn/blob/main/code/music-theory-ga/recheck/probe.cs), which calls the tool method directly: `Cdim7` → `{C, Eb, F#, A}`, 0 3 6 9, 4-28; `Cm7b5` → `{C, Eb, F#, Bb}`, 4-27, "Half Diminished Seventh". GA's server and GA's source now agree on both chords.
- A blank line before those two rows ended the QA table early, so they rendered as plain text; removed.
- The three rows about `FretDiagram` and `OptickIndexReader` were fixed upstream by [#696](https://github.com/GuitarAlchemist/ga/pull/696), merged 2026-09-23, each commit with its test: `FretDiagram.test.tsx` for the string order, `fretWindow.test.ts` for the dropped dots, `CompactVectorNormTests.cs` for the vector length.
- [#730](https://github.com/GuitarAlchemist/ga/pull/730), merged 2026-09-24, makes the export reproducible: the parallel generator now emits its windows in fret order, so the export's tie-break always keeps the same voicing. The two exports compared here, from one build, would no longer differ; the course's own index, exported from `66bdd049`, is unchanged.
