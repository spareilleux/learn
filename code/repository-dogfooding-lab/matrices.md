...
| Jev as an advisory hexavalent annotator | experimenting | inconclusive | Merge the division-of-labour rule into Demerzel's logic/hexavalent-logic.md (https://github.com/GuitarAlchemist/Demerzel/pull/1127); revisit only if a repository builds a model-on-evidence seam (gaia#159). | Advisory only: a Jev value is one input to Demerzel's deterministic confidence ladder, never a verdict; C still escalates to a human. |
