...
fn main() {
    ...
    let revenue: f64 = orders
        .iter()
        .map(|o| o.quantity as f64 * o.unit_price)
        .sum();
    let any_monitor = orders.iter().any(|o| o.product == "monitor");
    let all_positive = orders.iter().all(|o| o.quantity > 0);
    let first_mouse = orders
        .iter()
        .find(|o| o.product == "mouse")
        .map(|o| o.customer);
    println!(
        "revenue {revenue}, any monitor {any_monitor}, all positive {all_positive}, first mouse buyer {first_mouse:?}"
    );
    ...
    by_value.sort_by(|a, b| {
        (b.quantity as f64 * b.unit_price).total_cmp(&(a.quantity as f64 * a.unit_price))
    });
    ...
    println!(
        "expensive orders: {}",
        orders.iter().filter(|o| is_expensive(o)).count()
    );
